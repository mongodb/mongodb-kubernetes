...
#
# Behavior:
#   - waits (forever, with periodic status banner) for the file to
#     appear if it doesn't yet
#   - then runs `tail -F`, which follows by name and survives
#     truncation / rotation / replacement
#   - if tail exits for any reason (the symlink target changed, the
#     file got removed, parent dir disappeared, ...), prints a banner
