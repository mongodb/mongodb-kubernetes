...
func debugAppDB(ctx context.Context, opsManager *omv1.MongoDBOpsManager, centralClusterName string, memberCluster multicluster.MemberCluster, deployPods bool, diffwatchImage string) ([]attachCommand, error) {
	...
	for podIdx := 0; podIdx < opsManager.Spec.AppDB.GetMemberClusterSpecByName(memberCluster.Name).Members; podIdx++ {
		...
		sts := createAppDBStatefulSetObject(opsManager.Namespace, scriptsHash, templateData, deployPods, diffwatchImage)
...
func debugOM(ctx context.Context, opsManager *omv1.MongoDBOpsManager, reconcilerHelper operator.OpsManagerReconcilerHelper, centralClusterName string, memberCluster multicluster.MemberCluster, deployPods bool, diffwatchImage string) ([]attachCommand, error) {
	var attachCommands []attachCommand
	for podIdx := 0; podIdx < memberCluster.Replicas; podIdx++ {
		...
		sts := createOMDeploymentObject(opsManager.Namespace, scriptsHash, templateData, deployPods, diffwatchImage)
		if err = createStatefulSet(ctx, sts, memberCluster.Client); err != nil {
			return nil, xerrors.Errorf("error creating statefulset in cluster %s: %w", memberCluster.Name, err)
		}
...
func newAttachCommand(templateData TemplateData, centralClusterName string, memberClusterName string) attachCommand {
	debugStsName := debugStsName(templateData.PodName)
	debugPodName := fmt.Sprintf("%s-0", debugStsName)
...
func createAppDBStatefulSetObject(namespace string, scriptsHash string, templateData TemplateData, deployPods bool, diffwatchImage string) appsv1.StatefulSet {
...
func createOMDeploymentObject(namespace string, scriptsHash string, templateData TemplateData, deployPods bool, diffwatchImage string) appsv1.StatefulSet {
	deploymentName := fmt.Sprintf("mdb-debug-%s", templateData.PodName)
