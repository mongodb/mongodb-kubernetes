...
get_file() {
  ...
  kubectl exec -n "${namespace}" "${pod_name}" -c "${container_name}" -- cat "${src_file_path}" > "${dst_file_path}.tmp" || {
    echo "failed to get file ${pod_name}:${src_file_path} to ${dst_file_path}.tmp"
    return 1
  }
  mv "${dst_file_path}.tmp" "${dst_file_path}"
...
trim_config() {
  cfg_file=$1
  jq '. | walk(if type == "object" then with_entries(select(.key | test("Versions?$") | not)) else . end) | del(.clientPIT) | .' <"${cfg_file}"
}
...
dump_current_processes_in_pod() {
  ns=$1
  pod_name=$2
  container=$3

  # Execute kubectl command and capture output
  output=$(kubectl exec "${pod_name}" -n "${ns}" -c "${container}" -- ps -eo pid,args -ww 2>/dev/null)

  # Check if command was successful
  if [ $? -eq 0 ]; then
      # Process the output and convert to JSON
      echo "$output" | awk 'NR>1 {
          # Extract PID (first field) and COMMAND (rest of the line)
          pid = $1
          cmd = ""
          for (i=2; i<=NF; i++) {
              cmd = cmd (i==2 ? "" : " ") $i
...
monitor_processes_in_pod() {
  ...
  while true; do
      dump_current_processes_in_pod "${ns}" "${pod_name}" "${container}" >"${output_file}.tmp"
...
install_lnav_formats() {
  lnav -i https://github.com/lsierant/lnav-formats.git 2>/dev/null || true
}
