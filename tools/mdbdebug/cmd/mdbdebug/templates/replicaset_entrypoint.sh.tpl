...
if [[ "${static_arch}" == "true" ]]; then
  ...
  mongosh_path='/var/lib/mongodb-mms-automation/mongosh-linux-*/bin/mongosh'
fi
...
processes_file="${base_dir}/processes/processes.json"
...
mkdir -p "$(dirname "${processes_file}")"
...
monitor_processes_in_pod "{{.Namespace}}" "{{.PodName}}" "${container_name}" "${processes_file}" &
...
while true; do
  ...
  if [[ -n ${agent_api_key} ]]; then
    ...
    trim_config "${cluster_config_file}.tmp" > "${cluster_config_file}.trimmed.tmp"
    mv "${cluster_config_file}.trimmed.tmp" "${cluster_config_file}"
  fi
  ...
  if [[ "${static_arch}" == "true" ]]; then
    ...
    cmd="\$(ls ${mongosh_path} 2>/dev/null | head -1) --host {{.PodFQDN}} --port ${mongod_port} --eval 'JSON.stringify(rs.config())' --quiet"
  fi
  ...
  if [[ "${static_arch}" == "true" ]]; then
    ...
    cmd="\$(ls ${mongosh_path} 2>/dev/null | head -1) --host {{.PodFQDN}} --port ${mongod_port} --eval 'JSON.stringify(db.hello())' --quiet"
  fi
