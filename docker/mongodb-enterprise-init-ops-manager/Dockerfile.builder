...
WORKDIR /go/src
ADD . .
RUN CGO_ENABLED=0 go build -a -i -o /data/scripts/mmsconfiguration ./mmsconfiguration
RUN CGO_ENABLED=0 go build -a -i -o /data/scripts/backup-daemon-readiness-probe ./backupdaemon_readinessprobe/
