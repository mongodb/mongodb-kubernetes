...
class TestWebhookValidation(KubernetesTester):
    ...
    def test_duplicate_cluster_names(
        ...
        resource["spec"]["clusterSpecList"].append(
            {"clusterName": "e2e.cluster1.mongokubernetes.com", "members": 1}
        )

        self.create_custom_resource_from_object(
            self.get_namespace(),
            resource,
            exception_reason="Multiple clusters with the same name (e2e.cluster1.mongokubernetes.com) are not allowed",
