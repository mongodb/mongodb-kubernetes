...
def ops_manager(namespace: str, custom_version: Optional[str]) -> MongoDBOpsManager:
    ...
    # start at community version of MongoDB
    om.set_appdb_version("4.0.20")
    return om.create()


@mark.e2e_operator_upgrade_appdb_community_to_enterprise
...
@mark.e2e_operator_upgrade_appdb_community_to_enterprise
def test_upgrade_to_enterprise_edition(ops_manager: MongoDBOpsManager):

    # we cannot directly transition from community to enterprise with the new architecture so we need to
    # upgrade to the desired enterprise version.
    ops_manager.load()
    ops_manager["spec"]["applicationDatabase"]["version"] = "4.0.20-ent"
    ops_manager.update()
