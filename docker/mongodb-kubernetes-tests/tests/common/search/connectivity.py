...
def run_periodically_logged(
    ...
    *,
    ...
    """Logger-aware variant of ``kubetester.run_periodically``.

    ``fn`` returns ``bool`` or ``(bool, str)``; polls until True or timeout.
    """
    start = time.monotonic()
    deadline = start + timeout if timeout > 0 else float("inf")
    attempts = 0
    ...
    while time.monotonic() < deadline:
        ...
        if isinstance(result, tuple) and len(result) == 2:
            ...
            cond, status = result, None
        else:
            raise TypeError("fn must return bool or (bool, str)")
        if cond:
            elapsed = time.monotonic() - start
            logger.info(f"{name} succeeded after {elapsed:.2f}s and {attempts} attempt(s)")
            return True
        if msg is not None:
            tail = f": {status}" if status else ""
            logger.info(f"waiting for {msg}{tail}...")
        time.sleep(sleep_time)
    elapsed = time.monotonic() - start
    target = msg or name
    raise AssertionError(f"Timed out waiting for {target} after {elapsed:.2f}s and {attempts} attempt(s)")
...
class QueryResult:
    ...
    def __str__(self) -> str:
        bits = [f"page={self.page_index}", f"ok={self.success}", f"n={self.returned_count}", f"lat={self.latency_ms:.1f}ms"]
...
def list_pods_by_prefix(core_v1, namespace: str, prefix: str) -> list:
    """Return all pods in ``namespace`` whose name starts with ``prefix``."""
    from kubetester import _list_matching_pods

    return _list_matching_pods(namespace, name_prefix=prefix)


def hard_kill_pods_by_prefix(core_v1, namespace: str, prefix: str) -> dict[str, str]:
    """Hard-kill (grace=0) every pod whose name starts with ``prefix``.

    Returns ``{pod_name: original_uid}`` for use with ``wait_for_all_pods_replaced``.
    """
    from kubernetes import client as _k8s_client

    pods = list_pods_by_prefix(core_v1, namespace, prefix)
    if not pods:
        raise AssertionError(f"no pods with prefix {prefix!r} in ns {namespace}")
...
def hard_kill_pods_by_label(core_v1, namespace: str, label_key: str, label_value: str) -> dict[str, str]:
    """Hard-kill every pod whose labels match ``label_key=label_value``."""
    from kubernetes import client as _k8s_client

    pods = [
        p
        for p in core_v1.list_namespaced_pod(namespace=namespace).items
        if p.metadata.labels and p.metadata.labels.get(label_key) == label_value
    ]
    if not pods:
        raise AssertionError(f"no pods with label {label_key}={label_value} in ns {namespace}")
    original_uids: dict[str, str] = {}
    for p in pods:
        original_uids[p.metadata.name] = p.metadata.uid
        core_v1.delete_namespaced_pod(
...
def delete_pods_by_label(
    ...
    *,
    ...
    """Delete pods matching ``label_key=label_value`` respecting their grace period."""
    from kubernetes import client as _k8s_client
    ...
    if not pods:
        raise AssertionError(f"no pods with label {label_key}={label_value} in ns {namespace}")
    body = (
        _k8s_client.V1DeleteOptions(grace_period_seconds=grace_period_seconds)
    ...
    for p in pods:
        ...
        logger.info(f"deleted pod {p.metadata.name} (uid={p.metadata.uid[:8]})")
    return original_uids
...
def wait_for_pods_by_label_replaced(
    ...
    *,
    ...
    For Deployment-owned pods, replacements get fresh names — match by uid set
    rather than by name.
    """
    ...
    def check() -> tuple[bool, str]:
        live = [
            ...
            and p.metadata.labels.get(label_key) == label_value
            and p.metadata.deletion_timestamp is None
        ]
        new = [p for p in live if p.metadata.uid not in old_uid_set]
        ready = sum(
            1 for p in new if any(c.type == "Ready" and c.status == "True" for c in (p.status.conditions or []))
        )
    ...
    run_periodically_logged(
        ...
        timeout=timeout,
        sleep_time=3,
        msg=f"{want} new pods with {label_key}={label_value} Ready",
    )
...
def patch_mongot_readiness_probe_to_false(
    apps_v1, namespace: str, sts_name: str, container_name: str = "mongot"
) -> None:
    ...
    apps_v1.patch_namespaced_stateful_set(name=sts_name, namespace=namespace, body=patch)
    logger.info(f"patched {sts_name} container={container_name} readinessProbe -> /bin/false")


def restore_mongot_readiness_probe(apps_v1, namespace: str, sts_name: str, container_name: str = "mongot") -> None:
...
def wait_for_pods_not_ready(
    core_v1, namespace: str, prefix: str, *, expected: Optional[int] = None, timeout: int = 180
) -> None:
    from kubetester import wait_for_no_pods_ready

    wait_for_no_pods_ready(
        namespace,
        name_prefix=prefix,
        expected_not_ready=expected,
        timeout=timeout,
    )


def wait_for_pods_ready(
    core_v1, namespace: str, prefix: str, *, expected: Optional[int] = None, timeout: int = 300
) -> None:
    from kubetester import wait_for_pods_ready as _wait

    _wait(namespace, name_prefix=prefix, expected_count=expected, timeout=timeout)
