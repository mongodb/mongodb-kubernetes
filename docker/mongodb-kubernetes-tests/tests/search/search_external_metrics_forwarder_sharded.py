...
def assert_hosts(om: MongoDBOpsManager, mdbs: MongoDBSearch, shard_count: int):
    ...
    The shard names follow the source MongoDB's ShardRsNames() pattern ({mdb-name}-{idx}) while each
    shard's mongot pods are addressed via the MongoDBSearch's per-shard headless Service, and the Ops
    Manager project belongs to the source MongoDB.

    Adding/removing a shard or scaling a shard's mongots triggers an asynchronous metrics-forwarder
    reconcile that registers/deregisters the affected mongot host(s). We therefore poll until the
    registered MONGOT hosts match the expected set exactly rather than asserting once: a count-only
    check can pass transiently while membership is still wrong (a not-yet-registered new host masked by
    a not-yet-removed old host).
    """
    tester = om.get_om_tester(project_name=MDB_RESOURCE_NAME)

    replicas = mdbs["spec"]["clusters"][0].get("replicas", 1)
    expected_hostnames = {
        f"{shard_statefulset_name(mdbs.name, f'{MDB_RESOURCE_NAME}-{shard_idx}')}-{i}."
        f"{shard_service_name(mdbs.name, f'{MDB_RESOURCE_NAME}-{shard_idx}')}.{mdbs.namespace}.svc.cluster.local"
        for shard_idx in range(shard_count)
