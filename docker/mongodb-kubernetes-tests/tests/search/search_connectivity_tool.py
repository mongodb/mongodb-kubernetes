...
class TestSearchConnectivityTool(SearchE2EFixtures):
    ...
    def test_paging_through_mongot_outage_surfaces_connectivity_error(self, mdb: MongoDB, mdbs: MongoDBSearch):
        ...
        mdbs.update()
        wait_for_mongot_statefulset_drained(client.AppsV1Api(), statefulset_name, mdb.namespace)
    ...
    def test_paging_through_mongot_pod_restart_surfaces_lost_cursor(self, mdb: MongoDB, mdbs: MongoDBSearch):
        ...
        mongot_label = f"{search_resource_names.mongot_statefulset_name(mdbs.name)}-svc"

        _wait_for_search_serving(tool, timeout=200.0)

        def fault() -> None:
            uids = hard_kill_pods_by_label(core_v1, mdb.namespace, "app", mongot_label)
            assert (
                len(uids) == 1
            ), f"expected exactly 1 mongot pod matching app={mongot_label}; got {len(uids)}: {list(uids)}"
            _wait_for_search_serving(tool, timeout=200.0)
    ...
    def test_mongot_graceful_shutdown_with_active_cursor_holds_to_grace_and_cursor_dies_with_kill(
        ...
        old_pod = _single_mongot_pod(core_v1, mdb.namespace, mongot_label)
        ...
        try:
            pre_sigterm_pages = tool.paging_cursor_read_pages(
                cursor, pages=2, interval_seconds=0.1, batch_size=10, first_page_index=0,
            )
            for pp in pre_sigterm_pages:
                logger.info(f"pre-replacement page {pp.page_index}: {pp!s}")

            t_sigterm = datetime.now(timezone.utc)
            captured_uids = delete_pods_by_label(core_v1, mdb.namespace, "app", mongot_label)
            ...
            wait_for_all_pods_replaced(core_v1, mdb.namespace, captured_uids, timeout=180)
            _wait_for_search_serving(tool)
            ...
            for batch_start in range(0, max_post_pages, page_batch):
                batch = tool.paging_cursor_read_pages(
                    cursor, pages=page_batch, interval_seconds=0.1, batch_size=10,
    ...
    def test_mongot_graceful_shutdown_exits_early_after_killcursors(self, mdb: MongoDB, mdbs: MongoDBSearch):
        ...
        try:
            ...
            logger.info(f"deleting mongot pods app={mongot_label} (t_delete={t_delete.isoformat()})")
            captured_uids = delete_pods_by_label(core_v1, mdb.namespace, "app", mongot_label)
        ...
        t_old_exit = _watch_mongot_termination(core_v1, mdb.namespace, old_pod, watch_for=grace + 30)
        logger.info(f"OLD pod terminated at {t_old_exit.isoformat()}")
        wait_for_all_pods_replaced(core_v1, mdb.namespace, captured_uids, timeout=120)
        ...
        assert elapsed_from_delete < grace - 5, (
            f"OLD exited {elapsed_from_delete:.2f}s after delete; expected < grace-5 = {grace - 5}s"
        )

    def test_paging_through_envoy_restart_surfaces_lost_cursor(
        ...
        logger.info(f"envoy-restart test: deployment={envoy_dep} namespace={namespace}")
        ...
        def fault() -> None:
            killed = hard_kill_pods_by_label(core_v1, namespace, "app", envoy_dep)
            captured_uids.update(killed)
            wait_for_pods_by_label_replaced(
                core_v1, namespace, "app", envoy_dep, captured_uids, timeout=180,
            )
            _wait_for_search_serving(tool, timeout=200.0)

        _, _, verdict = paging_baseline_and_fault(tool, fault_fn=fault)
        assert verdict.cursor_lost > 0, (
    ...
    def test_direct_secondary_paging_succeeds(self, mdb: MongoDB):
        ...
        tester = get_rs_search_tester_for_member(
            mdb, member_index=1, username=cfg.user_name, password=cfg.user_password, use_ssl=True,
        )
    ...
    def test_direct_secondary_concurrent_with_primary_paging(self, mdb: MongoDB):
        ...
        try:
            for i in range(5):
                primary_pages.extend(primary_tool.paging_cursor_read_pages(
                    primary_cursor, pages=1, interval_seconds=0.0, batch_size=10, first_page_index=i,
                ))
                secondary_pages.extend(secondary_tool.paging_cursor_read_pages(
                    secondary_cursor, pages=1, interval_seconds=0.0, batch_size=10, first_page_index=i,
                ))
        finally:
...
def _wait_for_search_serving(
    ...
    *,
    ...
    if read_only:
        ...
        run_periodically(search_returns, timeout=timeout, sleep_time=1.0,
                         msg="secondary mongod→envoy→mongot path to serve $search")
        return
    tool.wait_for_sentinel_indexed(timeout=timeout)


def _single_mongot_pod(core_v1, namespace: str, label_value: str):
    """Return the ordinal-0 mongot pod for ``label_value`` (deterministic across rescales)."""
    pods = core_v1.list_namespaced_pod(namespace, label_selector=f"app={label_value}").items
    pods = [p for p in pods if p.metadata.name[p.metadata.name.rfind("-") + 1:].isdigit()]
    if not pods:
        raise AssertionError("expected at least 1 mongot pod, got 0")
...
def _page_for(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        batch = tool.paging_cursor_read_pages(
            cursor, pages=1, interval_seconds=0.0, batch_size=10, first_page_index=idx,
        )
        for pp in batch:
...
def _check_old_mongot_terminated(core_v1, namespace: str, old_name: str, old_uid: str) -> Optional[datetime]:
    try:
        pod = core_v1.read_namespaced_pod(name=old_name, namespace=namespace)
...
def _page_until_mongot_terminates(
    *,
    ...
    while time.monotonic() < deadline:
        ...
        terminated_at = _check_old_mongot_terminated(core_v1, namespace, old_name, old_uid)
...
def _watch_mongot_termination(core_v1, namespace: str, old_pod, *, watch_for: float) -> datetime:
    from tests.common.search.connectivity import run_periodically_logged
    ...
    def terminated() -> tuple[bool, str]:
        at = _check_old_mongot_terminated(core_v1, namespace, old_name, old_uid)
        if at is not None:
            captured["at"] = at
    ...
    run_periodically_logged(
        terminated, timeout=watch_for, sleep_time=0.5,
