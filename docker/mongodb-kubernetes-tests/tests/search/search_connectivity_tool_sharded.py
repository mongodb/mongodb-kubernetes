...
class TestSearchConnectivityToolSharded(
    ...
    def test_query_fails_when_envoy_endpoints_removed_for_one_shard(
        ...
        original_pod_count = len(list_pods_by_prefix(core_v1, namespace, pod_prefix))
        ...
        try:
            ...
            patch_mongot_readiness_probe_to_false(apps_v1, namespace, sts_name)
            probe_patched = True
            hard_kill_pods_by_prefix(core_v1, namespace, pod_prefix)
            wait_for_pods_not_ready(core_v1, namespace, pod_prefix, timeout=180)
            ...
            if probe_patched:
                restore_mongot_readiness_probe(apps_v1, namespace, sts_name)
            hard_kill_pods_by_prefix(core_v1, namespace, pod_prefix)
            if annotation_set:
