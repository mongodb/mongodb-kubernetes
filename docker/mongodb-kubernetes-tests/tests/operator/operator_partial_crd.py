...
def ops_manager_and_mongodb_crds():
    ...
    # apply_*_crd waits for the CRD to be Established before we create the CR below;
    ...
    # The operator reads the MemberCluster CRD at startup, so it must exist even here.
    apply_member_cluster_crd(api_client=client.api_client.ApiClient())
...
def mongodb_crds():
    ...
    # test_remove_operator_and_crds deletes the CRDs, so the API server must re-register the endpoint.
    apply_operator_config_crd(api_client=client.api_client.ApiClient())
    # The operator reads the MemberCluster CRD at startup, so it must exist even here.
    apply_member_cluster_crd(api_client=client.api_client.ApiClient())
...
def test_only_ops_manager_and_mongodb_crds_exist():
    ...
    assert len(operator_crds) == 4
    assert operator_crds[0].metadata.name == "memberclusters.operator.mongodb.com"
    assert operator_crds[1].metadata.name == "mongodb.mongodb.com"
    assert operator_crds[2].metadata.name == "operatorconfigs.operator.mongodb.com"
    assert operator_crds[3].metadata.name == "opsmanagers.mongodb.com"
