...
class TestCreateMongoDBUser(KubernetesTester):
    ...
    @classmethod
    def setup_class(cls):
        print(f"creating password for MongoDBUser {USER_NAME} in secret/{PASSWORD_SECRET_NAME} ")  # lgtm[py/clear-text-logging-sensitive-data]
...
class TestCanChangePassword(KubernetesTester):
    @classmethod
    def setup_env(cls):
        print(f"updating password for MongoDBUser {USER_NAME} in secret/{PASSWORD_SECRET_NAME}")  # lgtm[py/clear-text-logging-sensitive-data]
