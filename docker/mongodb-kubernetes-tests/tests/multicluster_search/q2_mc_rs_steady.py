...
def test_verify_per_cluster_status(mdbs: MongoDBSearch):
    ...
    NOTE: Currently dead — depends on the per-cluster status surface
    (SearchClusterStatusItem, Status.ClusterStatusList,
    AggregateClusterStatuses) that was cut from MongoDBSearch MVP scope
    upstream in PR #1074 on origin/search/ga-base. This assertion is
    preserved on branch `mc-search-status-reintroduction` as the
    acceptance test for the post-MVP reintroduction of per-cluster
    status reporting.
    """
    mdbs.load()
