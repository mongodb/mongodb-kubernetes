...
def test_create_oidc_replica_set(mongodb_multi: MongoDBMulti | MongoDB):
    mongodb_multi.assert_reaches_phase(Phase.Running, timeout=800)


def test_create_user(oidc_user: MongoDBUser):
    oidc_user.assert_reaches_phase(Phase.Updated, timeout=800)


def test_assert_connectivity(mongodb_multi: MongoDBMulti | MongoDB):
    tester = mongodb_multi.tester()
    tester.assert_oidc_authentication()


def test_ops_manager_state_updated_correctly(mongodb_multi: MongoDBMulti | MongoDB):
