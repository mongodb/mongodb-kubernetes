...
def mongodb_multi(
    ...
    resource = MongoDB.from_yaml(yaml_fixture("mongodb-multi-pvc-resize.yaml"), RESOURCE_NAME, namespace)
...
@pytest.mark.e2e_mongodb_multi_cluster_pvc_resize
def test_mongodb_multi_resize_pvc_state_changes(mongodb_multi: MongoDB):
    testhelper.test_mongodb_multi_resize_pvc_state_changes(mongodb_multi)


@pytest.mark.e2e_mongodb_multi_cluster_pvc_resize
def test_mongodb_multi_resize_finished(
    mongodb_multi: MongoDB, namespace: str, member_cluster_clients: List[MultiClusterClient]
):
