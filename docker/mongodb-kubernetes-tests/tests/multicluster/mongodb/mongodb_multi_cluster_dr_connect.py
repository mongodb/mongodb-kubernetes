...
# in the "cluster_host_mapping" fixture before running it. It is intented to be run locally with the command: make e2e-telepresence test=e2e_mongodb_multi_cluster_dr local=true
@pytest.fixture(scope="module")
def mongodb_multi(central_cluster_client: kubernetes.client.ApiClient, namespace: str) -> MongoDB:
    resource = MongoDB.from_yaml(yaml_fixture("mongodb-multi-dr.yaml"), MDB_RESOURCE, namespace)
...
@pytest.fixture(scope="module")
def mongodb_multi_collection(mongodb_multi: MongoDB):
    collection = mongodb_multi.tester().client["testdb"]
    return collection["testcollection"]


@pytest.mark.e2e_mongodb_multi_cluster_dr
...
@pytest.mark.e2e_mongodb_multi_cluster_dr
def test_replica_set_is_reachable(mongodb_multi: MongoDB):
...
@pytest.mark.e2e_mongodb_multi_cluster_dr
def test_replica_set_is_reachable_after_deletetion(mongodb_multi: MongoDB):
