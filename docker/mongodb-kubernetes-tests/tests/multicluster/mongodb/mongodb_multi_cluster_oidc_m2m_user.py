...
def mongodb_multi(
    ...
    resource = MongoDB.from_yaml(yaml_fixture("oidc/mongodb-multi-m2m-user.yaml"), MDB_RESOURCE, namespace)
...
@pytest.mark.e2e_mongodb_multi_cluster_oidc_m2m_user
class TestOIDCMultiCluster(KubernetesTester):
    ...
    def test_create_oidc_replica_set(self, mongodb_multi: MongoDB):
    ...
    def test_create_user(self, oidc_user: MongoDBUser):
        testhelper.TestOIDCMultiCluster.test_create_user(self, oidc_user)

    def test_assert_connectivity(self, mongodb_multi: MongoDB):
        testhelper.TestOIDCMultiCluster.test_assert_connectivity(self, mongodb_multi)

    def test_ops_manager_state_updated_correctly(self, mongodb_multi: MongoDB):
