...
@pytest.mark.e2e_mongodb_multi_cluster_replica_set_deletion
...
@pytest.mark.e2e_mongodb_multi_cluster_replica_set_deletion
def test_automation_config_has_been_updated(mongodb_multi: MongoDB):
    testhelper.test_automation_config_has_been_updated(mongodb_multi)


@pytest.mark.e2e_mongodb_multi_cluster_replica_set_deletion
def test_delete_mongodb_multi(mongodb_multi: MongoDB):
...
def test_kubernetes_resources_have_been_cleaned_up(
    mongodb_multi: MongoDB, member_cluster_clients: List[MultiClusterClient]
):
