...
def mongodb_multi_b_unmarshalled(
    ...
    resource = MongoDB.from_yaml(yaml_fixture("mongodb-multi.yaml"), "multi-replica-set", mdbb_ns)
    resource["spec"]["clusterSpecList"] = cluster_spec_list(member_cluster_names, [2, 1])
    resource.set_version(ensure_ent_version(custom_mdb_version))
...
def server_certs_a(
    ...
    mongodb_multi_a_unmarshalled: MongoDB,
...
def server_certs_b(
    multi_cluster_clusterissuer: str,
    mongodb_multi_b_unmarshalled: MongoDB,
    member_cluster_clients: List[MultiClusterClient],
    central_cluster_client: kubernetes.client.ApiClient,
):
...
@pytest.mark.e2e_mongodb_multi_cluster_2_clusters_clusterwide
def test_enable_mongodb_multi_nsa_auth(mongodb_multi_a: MongoDB):
