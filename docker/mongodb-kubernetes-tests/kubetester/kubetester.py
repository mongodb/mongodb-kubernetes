...
# Features cut from the decentralized POC (KUBE-153): deletion cleanup (CR deletion tears down
# neither directives, StatefulSets nor the automation config) and OM project switching (operators
# never create projects — pre-provisioning contract). Each use site says which one applies.
skip_if_decentralized = pytest.mark.skipif(
    os.getenv("DECENTRALIZED_E2E") == "true", reason="not part of the decentralized POC"
)
