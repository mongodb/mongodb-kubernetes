"""argparse-based dispatcher. The **only** module that catches
``WtCtlError`` (per plan).
"""
...
def cmd_status_all(runner: Runner, args: argparse.Namespace) -> int:
    """Repo-wide view. Walks `git worktree list` from the main repo and joins
    with network registry + EVG host data.
    """
    # Find the main repo root: cwd's git common dir parent. We can't rely on
    # resolve_worktree() because we may be outside any worktree.
    main_repo = _resolve_main_repo(runner)
    # For status --all we need *some* worktree's scripts/dev to invoke
    # dc_select_network.sh. Try the cwd first; fall back to main_repo.
    script_host = _resolve_script_host(main_repo)
    wt_dom = WorktreeDomain(runner)
    cp_dom = ComposeDomain(runner)
    nw_dom = NetworkDomain(runner, script_host)
    ev_dom = EvgDomain(runner, script_host)
    ...
    for entry in net_entries:
        ...
        if entry.status == "active":
            # Active per registry but no matching worktree on this side ->
            # that's the textbook orphan condition.
            orphans.append(
                OrphanRegistration(
...
def _resolve_script_host(main_repo: Path) -> Path:
    """Pick a directory whose ``scripts/dev/`` exists. The main repo's
    .git-parent typically *is* a worktree (with checked-out files); if not,
    fall back to cwd. (Phase-1 status --all ignores network domain when
    no scripts/dev/ is reachable — see callers.)
    """
    cwd = Path.cwd().resolve()
    if (cwd / "scripts" / "dev" / "dc_select_network.sh").is_file():
        return cwd
    if (main_repo / "scripts" / "dev" / "dc_select_network.sh").is_file():
        return main_repo
    return cwd  # network calls will gracefully degrade
...
def cmd_logs(refs: WorktreeRefs, args: argparse.Namespace) -> int:
    ...
    if not log_path.is_file():
        # Try the legacy `dc_<phase>.log` naming used by existing wrappers.
        legacy = logs_dir(refs.worktree_root) / f"dc_{phase}.log"
        if legacy.is_file():
            log_path = legacy
        else:
            sys.stderr.write(
...
def cmd_evg(runner: Runner, refs: WorktreeRefs, args: argparse.Namespace) -> int:
    ...
    if sub == "kubeconfig":
        ...
        return 0
    if sub == "terminate":
        host_id = args.host_id or _resolve_host_id(ev, refs)
        if not host_id:
...
def _resolve_host_id(ev: EvgDomain, refs: WorktreeRefs) -> Optional[str]:
    st = ev.state_for(refs.worktree_root)
...
def _read_context_env(worktree_root: Path) -> dict[str, str]:
    ...
    instantiate OmDomain just to read NAMESPACE.
    """
    out: dict[str, str] = {}
