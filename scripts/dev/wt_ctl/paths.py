...
def resolve_worktree(runner: Runner, cwd: Optional[Path] = None) -> WorktreeRefs:
    ...
    Raises ``NotInWorktree`` if cwd is not inside a worktree. The plan says
    verbs that target the current worktree must error with a hint when this
    happens; cli.py renders the hint.
    """
