...
Single concise key-value format per the plan. ANSI auto: enabled only when
stdout is a tty AND ``color != 'never'``. The ``KVTable`` helper emits
right-padded keys for alignment.
"""
