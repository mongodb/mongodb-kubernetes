...
class WorktreeDomain:
    ...
    def add(self, repo_root: Path, branch: str, *, force: bool = False) -> None:
        """Wrap ``scripts/dev/create_worktree.sh`` (preserves init flow)."""
        argv = [str(repo_root / "scripts/dev/create_worktree.sh")]
        if force:
            argv.append("-f")
        argv.append(branch)
        # create_worktree.sh assumes PROJECT_DIR is set; callers in this repo
        # source devenv before running it. Set it explicitly to repo_root.
