...
``WtCtlError`` (per plan); everything else raises and lets the CLI translate.
"""
...
class WtCtlError(Exception):
    ...
    # Default to 1 (command failure). Subclasses for misuse / preconditions
    # override to 2 (matches plan exit-code convention).
    exit_code: int = 1
...
class ExternalCommandFailed(WtCtlError):
    ...
    for the CLI / caller to render. We deliberately don't subclass via
    ``__init__`` -- dataclass + ``__post_init__`` keeps fields explicit.
    """
