...
if [[ ${skip_devcontainer} -eq 0 ]]; then
  echo
  echo "Attach a shell from the host:"
  echo "  devcontainer exec --workspace-folder \"${worktree_root}\" bash"
  echo
  echo "Attach the existing tmux session inside the container:"
  echo "  devcontainer exec --workspace-folder \"${worktree_root}\" tmux a -t mck"
fi
