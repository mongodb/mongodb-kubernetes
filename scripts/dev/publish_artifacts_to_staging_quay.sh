#!/bin/bash
#
# Script to publish container images and helm charts to staging registry
#
# Authentication:
#   - For ECR: Uses AWS CLI credentials (aws ecr get-login-password)
#   - For Quay.io: Uses docker login credentials OR set QUAY_USERNAME/QUAY_PASSWORD env vars
#   - Recommended: Run 'docker login quay.io' before using this script
#
...
# Function to get AMD64 digest for multi-arch images
get_arch_digest() {
    local image="$1"
    local arch="$2"
    local manifest_json
    ...
    # Not a manifest list or no AMD64 found
    echo ""
    return 0
}

# Function to process a single image using skopeo
process_image() {
    ...
    echo "digest_arm64=\"${digest_arm64}\", digest_amd64=\"${digest_amd64}\""
...
publish_images() {
  ...
  # Build arrays of all images to process
  NAMES=()
  SOURCES=()
  DESTINATIONS=()
  ...
  for image in "${STANDARD_IMAGES[@]}"; do
      ...
      DESTINATIONS+=("${STAGING_BASE_URL}/${image}:${operator_version}")
  done
...
push_helm_oci_image() {
  helm push --registry-config="~/.docker/config" "tmp/mongodb-kubernetes-${helm_chart_version}.tgz" "oci://${STAGING_BASE_URL}"
}
...
update_release_json() {
  ...
  jq --arg version "${operator_version}" \
  ...
  jq --arg searchVersion "${MDB_SEARCH_COMMUNITY_VERSION}" \
    '.search.community.version = $searchVersion' \
    release.json > release.json.tmp && mv release.json.tmp release.json

  echo "Updated release.json with:"
  echo "  - Operator versions: ${operator_version}"
