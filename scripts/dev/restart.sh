...
kubectl delete -f mongodb-enterprise-operator.yaml
...
if [ -z "${BUILD_LOCALLY}" ]; then
    ...
    echo "-- Building mongodb-enterprise-operator on Linux machine ($connect_to)"
    ssh $connect_to 'source ~/.profile ; cd go/src/github.com/10gen/ops-manager-kubernetes/ ; go build -o mongodb-enterprise-operator'
else
    echo "-- Cross compiling mongodb-enterprise-operator"
    CGO_ENABLED=0 GOOS=linux go build -i -o mongodb-enterprise-operator || exit 1
fi
...
kubectl apply -f mongodb-enterprise-operator-local.yaml
sleep 3
