...
# MCK repo dir (defaults to the repo this script lives in; only an absolute
# MCK_DIR override is honored, since relative paths break after `pushd ../`)
script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
if [[ -z "${MCK_DIR:-}" || "${MCK_DIR}" != /* ]]; then
  MCK_DIR="$(dirname "$(dirname "${script_dir}")")"
fi
# Docs repo dir
