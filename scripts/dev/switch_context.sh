...
# Unified two-step capture for both local-dev and EVG-CI:
#
#   Step (a) — site_envs: source site-context alone in an `env -i` subshell,
#              forwarding only the inputs site-context reads (PWD/PATH/HOME
#              for paths, the four wt-ctl stack vars + LOCAL_OPERATOR for the
#              KUBECONFIG/proxy decisions). The output is exactly the set
#              of "site-derived" bytes for the running side.
#
#   Step (b) — current_envs: source the full chain (site + local-defaults +
#              context_file + optional overrides) in a way that lets EVG
#              expansions (BUILD_VARIANT, EVR_TASK_ID, otel_*, etc.) flow
#              through. Locally we use a second `env -i` subshell; on EVG-CI
#              we run the chain in the current shell so expansion vars set
#              by evergreen.expansions.update() are visible to root-context
#              + the per-variant context file (they read TASK_NAME, IS_PATCH,
#              etc.). The historical motivation for keeping these two paths
#              split — PR #3115 — was specifically about (b)'s capture
#              strategy, NOT about the per-side file write that came later.
#              That per-side write is now unified below.
#
# After step (b), logical_envs = current_envs − site_keys. Both .env files
# get written. The EVR_TASK_ID-gated "no per-side split" branch is gone.
