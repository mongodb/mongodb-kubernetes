...
# Master-compat artifacts (deprecated but still emitted): older checkouts
# (e.g. a worktree from master, whose set_env_context.sh / create_worktree.sh
# read it) and the EVG YAMLs not yet migrated to set_env_context.sh source
# .generated/context.export.env / .generated/context.operator.export.env.
# Emit both alongside the side files so wt-ctl can drive worktrees that don't
# carry this tooling. The export.env is the merged view (logical + THIS side's
# site bytes) — side-last-writer wins, exactly master's semantics.
{
    echo -e "## This file is automatically generated by switch_context.sh\n## Do not edit it!"
