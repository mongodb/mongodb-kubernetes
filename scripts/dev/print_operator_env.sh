...
"
  ...
  if [[ "${MDB_MONARCH_IMAGE:-""}" != "" ]]; then
    echo "MDB_MONARCH_IMAGE=${MDB_MONARCH_IMAGE}"
  fi

  if [[ "${MANAGED_SECURITY_CONTEXT:-""}" != "" ]]; then
    echo "MANAGED_SECURITY_CONTEXT=${MANAGED_SECURITY_CONTEXT}"
  fi
