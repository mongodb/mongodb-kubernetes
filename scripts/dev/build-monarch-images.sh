...
# --skip-if-exists false forces a rebuild even when the :monarch tag already exists.
# --registry overrides build_info.json registries (which include quay.io secondary
# repos that we don't have credentials for in local dev).
SKIP_IF_EXISTS_FLAG="--skip-if-exists false"
ECR_STAGING="268558157000.dkr.ecr.us-east-1.amazonaws.com/staging"

# Build agent (background)
if needs_rebuild "agent" "$AGENT_COMMIT"; then
    build_image "agent" "$AGENT_COMMIT" \
        python scripts/release/pipeline.py agent -b staging --agent-path="$AGENT_PATH" --version monarch -p "$PLATFORM" $SKIP_IF_EXISTS_FLAG --registry "${ECR_STAGING}/mongodb-agent" &
    PIDS+=($!)
    NAMES+=("agent")
else
...
if needs_rebuild "ops-manager" "$OM_COMMIT"; then
    build_image "ops-manager" "$OM_COMMIT" \
        python scripts/release/pipeline.py ops-manager -b staging --om-path="$OM_PATH" --version monarch -p "$PLATFORM" $SKIP_IF_EXISTS_FLAG --registry "${ECR_STAGING}/mongodb-enterprise-ops-manager-ubi" &
    PIDS+=($!)
    NAMES+=("ops-manager")
else
...
if needs_rebuild "monarch-injector" "$MONARCH_DOCKERFILE_HASH"; then
    build_image "monarch-injector" "$MONARCH_DOCKERFILE_HASH" \
        python scripts/release/pipeline.py monarch-injector -b staging --version monarch -p "$PLATFORM" $SKIP_IF_EXISTS_FLAG --registry "${ECR_STAGING}/mongodb-kubernetes-monarch-injector" &
    PIDS+=($!)
