...
def _make_fixture(tmp: Path, *, with_compose_running: bool, with_evg_host: bool) -> tuple[Path, Path, FakeRunner]:
    ...
    if with_evg_host:
        # `.current-evg-host` pin signals EVG-host mode to the delete
        # orchestrator's _step_evg_terminate. Without it the dispatch
        # falls through to the local-kind / BYOC branch.
...
class DeletePipelineTests(unittest.TestCase):
    ...
    def test_local_kind_evg_step_deletes_kind_cluster(self) -> None:
        """No `.current-evg-host`? --evg reads the kubeconfig's
        `current-context`; if it starts with `kind-` and the cluster exists,
        `kind delete cluster --name <stripped>`.
        """
        with tempfile.TemporaryDirectory() as tmp:
            repo, target, runner = _make_fixture(
                Path(tmp),
            ...
            # `kind get clusters` returns the cluster we expect to delete.
    ...
    def test_byoc_evg_step_skips_when_context_not_kind(self) -> None:
        """If kubeconfig's current-context doesn't start with `kind-`, this
        is a BYOC worktree — we don't own the cluster, so --evg is a no-op
        (no kind delete, no evergreen terminate).
        """
        with tempfile.TemporaryDirectory() as tmp:
            repo, target, runner = _make_fixture(
                Path(tmp),
