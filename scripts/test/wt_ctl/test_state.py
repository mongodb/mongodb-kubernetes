...
* hash-mismatch detection (resume re-runs phases whose inputs changed),
* ``clear_from`` resets a phase + everything later (used by --restart-from).
"""
...
class HashMismatchTests(unittest.TestCase):
    ...
    def test_resume_uses_input_hash_to_invalidate(self) -> None:
        """If a phase was OK but the current input hash differs, resume must
        treat it as needing re-run. We simulate that with first_non_ok-like
        logic — the orchestrator's _plan calls input_hash() afresh and
        compares against rec.input_hash.
        """
        st = ostate.OrchestratorState.initial(branch="b", inputs={})
        st.set_status("worktree_init", ostate.OK, input_hash="hash-A")
        # Pretend the user changed inputs → new hash differs from stored one.
        new_hash = "hash-B"
        rec = st.phases["worktree_init"]
        self.assertEqual(rec.status, ostate.OK)
        self.assertNotEqual(rec.input_hash, new_hash)
