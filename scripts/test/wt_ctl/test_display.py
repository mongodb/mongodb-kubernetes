...
def _fixture() -> WorktreeStatus:
    return WorktreeStatus(
        ...
        git=GitState(
            branch="lsierant/devcontainer", clean=True, ahead=0, behind=0,
            upstream="origin/lsierant/devcontainer",
        ),
        context="e2e_smoke",
        devc=DevcState(
            project="lsierant_devcontainer_devcontainer",
            state="running", services_running=4, services_total=4, image_age="2h",
        ),
        network=NetState(
            prefix=28, subnet="172.28.0.0/16",
            namespace="mck-devc-28-mongodb-test",
            gost_proxy="http://127.0.0.1:8028",
            registered=True, orphans=0,
        ),
        evg=EvgHostState(
            name="lsierant_devcontainer", id="i-0abc", status="running",
        ...
        om=OmState(
            namespace="ls-28", project_count=2,
            scope="ls-28, ls-28-*",
        ),
...
class StatusRenderTests(unittest.TestCase):
    def test_keys_present(self) -> None:
        ...
        for k in ("worktree", "path", "context", "devc", "network",
                  "namespace", "gost-proxy", "evg host", "kubeconfig", "om", "next"):
...
class StatusAllRenderTests(unittest.TestCase):
    def test_basic_table(self) -> None:
        rows = [
            WorktreeRow(
                worktree="alpha", branch="topic/a", prefix=20,
                namespace="ns-20", devc_state="running",
                evg_name="alpha", evg_status="running", evg_expires="2h",
            ),
            WorktreeRow(
                worktree="beta", branch="topic/b", prefix=21,
                namespace="ns-21", devc_state="exited",
                evg_name=None, evg_status=None, evg_expires=None,
                prunable=True, prunable_reason="devc-exited",
                delete_cmd="wt-ctl delete topic/b",
            ),
        ]
        orphans = [
            OrphanRegistration(
                branch_dir="ghost", prefix=22,
