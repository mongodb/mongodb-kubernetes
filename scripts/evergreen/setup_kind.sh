...
if [[ "${arch_suffix}" == "amd64" || "${arch_suffix}" == "arm64" ]]; then
  ...
  curl --retry 10 --retry-delay 5 --retry-all-errors --fail --show-error --max-time 600 -L "https://github.com/kubernetes-sigs/kind/releases/download/${latest_version}/kind-${os}-${arch_suffix}" -o kind
