...
is_mdb_debug() {
    ...
    kubectl exec "${pod}" -n "${namespace}" -- ls /var/log/mongodb-mms-automation/mdb-debug &>/dev/null
}
...
dump_pod_logs() {
    ...
    if is_mdb_debug "${pod}" "${namespace}"; then
        echo "Found mdb-debug directory, compressing all files from ${pod} log directory"
        kubectl exec "${pod}" -n "${namespace}" -c mongodb-agent -- bash -c "mkdir -p /tmp" || true
        kubectl exec "${pod}" -n "${namespace}" -c mongodb-agent -- bash -c "tar --warning=no-file-changed -czf /tmp/${namespace}_${pod}-logs.tar.gz -C /var/log mongodb-mms-automation/" || true
        kubectl cp -c mongodb-agent "${namespace}/${pod}:/tmp/${namespace}_${pod}-logs.tar.gz" "logs/${prefix}${pod}-logs.tar.gz" || true
        kubectl exec "${pod}" -n "${namespace}" -c mongodb-agent -- bash -c "rm -rf /tmp/${namespace}_${pod}-logs.tar.gz" || true
    fi
