...
def _render_timeline_entry(idx: int, entry: Dict) -> str:
    ...
    if timestamp:
        time_display = (
            f"<span class='timeline-timestamp' title='{escape(str(timestamp))}'>{escape(str(timestamp)[11:19])}</span>"
        )
...
def _render_pods_enhanced(pods: list, log_tails: Dict, multi_cluster: bool, reference_time: datetime = None) -> str:
    ...
    for cluster in sorted(by_cluster.keys()):
        ...
        for idx, pod in enumerate(cluster_pods[:50], 1):
            ...
            all_files = (
                pod["files"].get("logs", []) + pod["files"].get("agent_logs", []) + pod["files"].get("config", [])
            )
            ...
            if containers:
                ...
                for c in containers:
                    ...
                    ready_html = (
                        "<span class='ready-yes'>&#10003;</span>" if ready else "<span class='ready-no'>&#10007;</span>"
                    )
                    restarts = c.get("restarts", 0)
                    exit_code = c.get("exit_code")
                    exit_html = (
                        f"<span class='exit-code'>{exit_code}</span>"
        ...
        if len(cluster_pods) > 50:
            html += (
                f"<tr><td colspan='8' class='truncation-note'>Showing first 50 of {len(cluster_pods)} pods.</td></tr>"
            )
        html += "</table>"
...
def _render_statefulsets_enhanced(
    statefulsets: list, log_tails: Dict, multi_cluster: bool, reference_time: datetime = None
) -> str:
    ...
    for cluster in sorted(by_cluster.keys()):
        ...
        for idx, sts in enumerate(cluster_items[:50], 1):
            ...
            if yaml_file and yaml_file in log_tails:
                html += (
                    f"<td><code class='file-tag-link' onclick=\"openLogModal('{escape(yaml_file)}')\">view</code></td>"
                )
            else:
                html += "<td>-</td>"
            html += "</tr>"
...
def _render_deployments_enhanced(
    deployments: list, log_tails: Dict, multi_cluster: bool, reference_time: datetime = None
) -> str:
...
def _render_generic_table(
    items: list, source_files: list, log_tails: Dict, multi_cluster: bool, reference_time: datetime = None
) -> str:
...
def _render_resource_tabs(data: Dict, log_tails: Dict, reference_time: datetime) -> str:
    ...
    for i, slug in enumerate(ordered_slugs):
        ...
        if slug == "pods" and data["resources"]["pods"]:
            ...
            html += _render_statefulsets_enhanced(
                data["resources"]["statefulsets"], log_tails, multi_cluster, reference_time
            )
        elif slug == "deployments" and data["resources"]["deployments"]:
            html += _render_deployments_enhanced(
                data["resources"]["deployments"], log_tails, multi_cluster, reference_time
            )
        elif slug == "secrets":
...
def generate_html(data: Dict[str, Any], test_name: str, variant: str, status: str) -> str:
    ...
    unhealthy_pods = sum(
        1 for pod in data["resources"]["pods"] if pod.get("phase") != "Running" or pod.get("restarts", 0) > 0
    )
    ...
    timeline_truncation = (
        f"<p class='truncation-note'>Showing first 200 of {len(data['timeline'])} events.</p>"
