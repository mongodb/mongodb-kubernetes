...
ERROR_PATTERNS = {
    ...
    "oom_killed": {"regex": r"OOMKilled", "severity": "critical", "description": "Out of memory - container killed"},
    "image_pull_error": {
    ...
    "failed_mount": {"regex": r"FailedMount", "severity": "critical", "description": "Volume mount failed"},
    "evicted": {"regex": r"Evicted", "severity": "critical", "description": "Pod evicted due to resource pressure"},
    "connection_refused": {"regex": r"connection refused", "severity": "error", "description": "Connection refused"},
    "connection_timeout": {
...
def get_compiled_patterns() -> List[Tuple[str, "re.Pattern[str]", dict]]:
    ...
    """
    return [(name, re.compile(info["regex"], re.IGNORECASE), info) for name, info in ERROR_PATTERNS.items()]
