...
func TestBuildStatefulSet_PersistentFlag(t *testing.T) {
	...
	set, err := DatabaseStatefulSet(*mdb, ReplicaSetOptions())
...
func TestBuildStatefulSet_PersistentVolumeClaimSingle(t *testing.T) {
	...
	set, err := DatabaseStatefulSet(*rs, ReplicaSetOptions())
...
func TestBuildAppDbStatefulSetDefault(t *testing.T) {
	appDbSts, err := AppDbStatefulSet(defaultOpsManagerBuilder().Build())
	assert.NoError(t, err)
...
func TestBasePodSpec_AntiAffinityDefaultTopology(t *testing.T) {
	sts, err := DatabaseStatefulSet(*mdbv1.NewStandaloneBuilder().SetName("my-standalone").Build(), StandaloneOptions())
	assert.NoError(t, err)
...
func TestBasePodSpec_ImagePullSecrets(t *testing.T) {
	...
	sts, err := DatabaseStatefulSet(*mdbv1.NewStandaloneBuilder().Build(), StandaloneOptions())
	assert.NoError(t, err)
...
func TestBasePodSpec_TerminationGracePeriodSeconds(t *testing.T) {
	sts, err := DatabaseStatefulSet(*mdbv1.NewReplicaSetBuilder().Build(), ReplicaSetOptions())
	assert.NoError(t, err)
...
func TestPodSpec_Requirements(t *testing.T) {
	...
	sts, err := DatabaseStatefulSet(*mdbv1.NewReplicaSetBuilder().SetPodSpec(&podSpec.MongoDbPodSpec).Build(), ReplicaSetOptions())
	assert.NoError(t, err)
