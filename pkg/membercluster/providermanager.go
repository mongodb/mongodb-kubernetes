...
func newProviderManager(baseCtx context.Context, c client.Reader, clientTimeout time.Duration, provider *multicluster.Provider, newCluster func(restConfig *restclient.Config) (cluster.Cluster, error)) *providerManager {
...
// loadCredentials reads the credential Secret referenced by the MemberCluster CR from
// the CR's namespace and derives the member cluster's rest.Config and a hash of the raw
// kubeconfig for rotation detection.
func (m *providerManager) loadCredentials(ctx context.Context, mc *operatorv1.MemberCluster) (*credentials, error) {
	...
	if err := m.client.Get(ctx, types.NamespacedName{Name: secretName, Namespace: mc.Namespace}, secret); err != nil {
