...
type ProgressSnapshot struct {
	...
	PendingError            string
	// CRSpecGeneration is the CR's metadata.generation observed by the
	// reporter at the time this snapshot was built. Carried into the FSM's
	// ComponentStatus so AcquireOrRespect can detect that a previously-Ready
	// state is stale (the spec has been updated since the last MarkReady)
	// and therefore must NOT short-circuit to LeaseOtherClusterDone. Zero ⇒
	// "not reported" (legacy / unknown); the gate treats zero as always
	// stale, which preserves backwards compatibility while forcing one extra
	// reconcile pass after an upgrade.
	CRSpecGeneration int64
}
...
type DistributedCoordinator interface {
	...
	//    component AND the recorded SpecGeneration is >= currentSpecGen
	//    (i.e. the prior Ready reflects the current spec, not a stale one).
	//    Caller should `continue` past this iteration.
	//
	// currentSpecGen is the CR's metadata.generation observed by the caller
	// for this reconcile. The gate uses it to invalidate stale Ready bits:
	// a previously-Ready (component, cluster) whose stored SpecGeneration is
	// older than currentSpecGen MUST NOT short-circuit — the spec has changed
	// and the STS needs to be re-constructed/applied. Pass 0 from callers
	// that don't track a CR generation (legacy / non-MongoDB resources); 0
	// is treated as "always equal" for backwards compatibility.
	AcquireOrRespect(crKey CRKey, component, cluster string, currentSpecGen int64) LeaseResult

	// IsComponentReady reads from FSM Statuses without proposing anything.
	// currentSpecGen is consulted the same way as AcquireOrRespect: a Ready
	// entry whose stored SpecGeneration is older than currentSpecGen is
	// reported as NOT ready, so callers re-do the work for the new spec.
	IsComponentReady(crKey CRKey, component, cluster string, currentSpecGen int64) bool
	...
	// GetLeasesHeldBy returns the components for which `cluster` currently
	// holds an active lease on `crKey`. Used by the reconciler's lease-keep-
	// alive path: when a reconcile blocks at a top-of-reconcile gate (e.g.
	// Gate 0 / CR-spec agreement), the caller iterates the returned
	// components and emits ReportProgress on each so the FSM-side
	// HeartbeatAt is refreshed and the leader's stuck-step detector does
	// not revoke an actively-held lease while the holder is genuinely
	// trying to advance. Returns nil for an unknown CR or a cluster that
	// holds no leases. Safe to call frequently; the underlying FSM read is
	// cheap (single map lookup under RLock).
	GetLeasesHeldBy(crKey CRKey, cluster string) []string
	...
	// PublishAgentKey announces the OM agent API key for a (CR, projectID)
	// pair across the cluster via raft. The leader calls this after it has
	// either read or generated the key against Ops Manager so followers can
	// learn the value from the FSM without making OM reads.
	// Idempotent: re-publishing the same key is a no-op.
	PublishAgentKey(crKey CRKey, projectID, agentKey string) error

	// GetAgentKey returns the FSM-stored agent API key for (CR, projectID),
	// or "" if the leader hasn't published one yet.
	GetAgentKey(crKey CRKey, projectID string) string

	// ReportCRStatus submits a CRKey-level heartbeat StatusReport on every
	// reconciler `updateStatus` exit so the leader's FSM has up-to-date
	// per-follower progress. The payload carries the calling cluster's
	// observed Phase ("Pending" / "Running" / "Failed" / etc.) and Message
	// in the FSM's existing LastReconcileErr field; ComponentStatus is left
	// empty so the report merges into existing per-component progress
	// without wiping it. Idempotent: replays are harmless (the FSM just
	// overwrites LastReportedAt / LastReconcileErr).
	//
	// Used by the leader's stuck-step detector to release leases held by
	// followers that have stopped emitting updateStatus events.
	ReportCRStatus(crKey CRKey, phase, message string) error
}
