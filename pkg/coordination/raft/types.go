...
type ManagerConfig struct {
	...
	// Production switches the base raft.Config from FastConfig (50ms timers,
	// "ERROR" logs — for in-process tests) to ProductionRaftConfig
	// (hashicorp/raft library defaults + "INFO" logs — for real TCP
	// transports across kind clusters via Istio mesh).
	//
	// Cross-cluster RTT alone often exceeds FastConfig's 50ms timeouts, so
	// elections in the PoC operator pod never finish. Always set Production
	// in BuildProductionCoordinator; unit tests using NewInmemTransportPool
	// keep the default (Production=false).
	Production bool
}
