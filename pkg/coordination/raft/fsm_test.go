...
func TestApplyLeaseAllocate_AndComplete(t *testing.T) {
	...
	// Allocating a DIFFERENT (component, cluster) slot succeeds — the
	// distributed-mode design supports parallel per-(component, cluster)
	// leases. Both must coexist in the FSM.
	r2 := applyHelper(t, f, ProposalLeaseAllocate, LeaseAllocatePayload{
		CRKey: crA, Component: "config", ClusterName: "cluster-b",
	})
	lease2, _ := r2.(*Lease)
	require.NotNil(t, lease2)
	assert.Equal(t, "config", lease2.Component)
	assert.Equal(t, "cluster-b", lease2.ClusterName)
	assert.NotNil(t, f.GetLease(crA, "shard-0", "cluster-a"))
	assert.NotNil(t, f.GetLease(crA, "config", "cluster-b"))

	// Re-allocating the same (component, cluster) slot is a no-op and
	// returns the existing lease unchanged.
	r3 := applyHelper(t, f, ProposalLeaseAllocate, LeaseAllocatePayload{
		CRKey: crA, Component: "shard-0", ClusterName: "cluster-a",
	})
	lease3, _ := r3.(*Lease)
	require.NotNil(t, lease3)
	assert.Equal(t, lease.AllocatedAt.UnixNano(), lease3.AllocatedAt.UnixNano(),
		"re-allocate of same slot must return the existing lease, not a new one")

	// Complete with non-matching slot: existing leases unchanged.
	applyHelper(t, f, ProposalLeaseComplete, LeaseCompletePayload{
		CRKey: crA, Component: "config", ClusterName: "cluster-elsewhere",
	})
	assert.NotNil(t, f.GetLease(crA, "shard-0", "cluster-a"))
	assert.NotNil(t, f.GetLease(crA, "config", "cluster-b"))

	// Complete the shard-0 slot: cleared, but config slot intact.
	applyHelper(t, f, ProposalLeaseComplete, LeaseCompletePayload{
		CRKey: crA, Component: "shard-0", ClusterName: "cluster-a",
	})
	assert.Nil(t, f.GetLease(crA, "shard-0", "cluster-a"))
	assert.NotNil(t, f.GetLease(crA, "config", "cluster-b"))

	// Complete the config slot too: both gone, ActiveLeases now empty.
	applyHelper(t, f, ProposalLeaseComplete, LeaseCompletePayload{
		CRKey: crA, Component: "config", ClusterName: "cluster-b",
	})
...
func TestApplyLeaseExpire(t *testing.T) {
	...
	require.NotNil(t, f.GetLease(crA, "config", "cluster-a"))
	...
	require.Nil(t, f.GetLease(crA, "config", "cluster-a"))
