...
// SweepLeases walks every PerCR entry's ActiveLease and proposes LeaseExpire
// if any of the four conditions hits. Returns the count of revokes proposed
// (useful for tests).
func (s *StuckDetector) SweepLeases() int {
	...
	for crStr, perCR := range state.PerCR {
		if perCR.ActiveLease == nil {
			continue
		}
		lease := perCR.ActiveLease
		// Heartbeat-TTL.
		...
		// Compute current progress signature for this scope.
		cs, hasCluster := perCR.PerClusterStatus[lease.ClusterName]
		if !hasCluster {
			// No status reports yet — keep waiting (warmup already passed).
			continue
		}
		...
		if now.Sub(prev.seenAt) > s.cfg.StuckThreshold {
			...
			delete(s.memo, key) // reset so a fresh allocation starts clean
			revoked++
		}
	}
