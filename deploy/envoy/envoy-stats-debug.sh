...
check_tools_pod() {
    ...
    if ! kubectl get pod "$TOOLS_POD" --context "${CLUSTER_NAME}" --namespace "$NAMESPACE" &>/dev/null; then
        ...
        kubectl wait --for=condition=Ready pod/"$TOOLS_POD" --context "${CLUSTER_NAME}" --namespace "$NAMESPACE" --timeout=60s
    fi
...
exec_curl() {
    ...
    kubectl exec "$TOOLS_POD" --context "${CLUSTER_NAME}" --namespace "$NAMESPACE" -- \
