...
OUTPUT_FILE="${1:-envoy-capture-$(date +%Y%m%d-%H%M%S)}"
...
kubectl exec --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -- cat /etc/envoy/tls/server/cert.pem > "./captures/${OUTPUT_FILE}-keys.pem" 2>/dev/null
kubectl exec --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -- cat /etc/envoy/tls/client/cert.pem >> "./captures/${OUTPUT_FILE}-keys.pem" 2>/dev/null
...
DEBUG_CONTAINER=$(kubectl get pod --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -o jsonpath='{.spec.ephemeralContainers[-1].name}')
...
if [ -n "${DEBUG_CONTAINER}" ]; then
    ...
    if kubectl cp "${NAMESPACE}/${ENVOY_POD}:/tmp/capture.pcap" "./captures/${OUTPUT_FILE}.pcap" --context "${CLUSTER_NAME}" -c "${DEBUG_CONTAINER}" 2>/dev/null; then
        ...
        if kubectl exec --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -c "${DEBUG_CONTAINER}" -- cat /tmp/capture.pcap > "./captures/${OUTPUT_FILE}.pcap" 2>/dev/null; then
            echo "Successfully copied via exec"
        else
            echo "Warning: Could not copy file automatically"
            echo "Manual copy command:"
            echo "  kubectl exec --context \"\${CLUSTER_NAME}\" --namespace ${NAMESPACE} ${ENVOY_POD} -c ${DEBUG_CONTAINER} -- cat /tmp/capture.pcap > ./captures/${OUTPUT_FILE}.pcap"
        fi
    fi
