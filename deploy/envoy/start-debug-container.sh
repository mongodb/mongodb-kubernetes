...
kubectl exec --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -- cat /etc/envoy/tls/server/cert.pem > "./captures/envoy-keys-${TIMESTAMP}.pem" 2>/dev/null
kubectl exec --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -- cat /etc/envoy/tls/client/cert.pem >> "./captures/envoy-keys-${TIMESTAMP}.pem" 2>/dev/null
...
kubectl debug --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" "${ENVOY_POD}" -it --image=nicolaka/netshoot -- bash
