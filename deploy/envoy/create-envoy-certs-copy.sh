...
if ! kubectl get configmap issuer-ca --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" &> /dev/null; then
...
kubectl get configmap issuer-ca --context "${CLUSTER_NAME}" --namespace "${NAMESPACE}" -o jsonpath='{.data.ca-pem}' > ca.pem
