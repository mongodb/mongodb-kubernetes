...
type podListingReconciler struct {
	referenceClusterClient, otherCluster client.Client
}
...
func (p podListingReconciler) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	fmt.Printf("Pod in the other cluster!\n")
	fmt.Printf("Found Pod: %s in namespace %s!\n", pod.Name, pod.Namespace)

	return reconcile.Result{}, nil
}

func NewPodListingReconciler(mgr manager.Manager, mirrorCluster cluster.Cluster) error {
...
func main() {
	...
	cfg2, err := clientcmd.BuildConfigFromFlags("", "/etc/config/kubeconfig/kubeconfig")
	if err != nil {
		panic(err)
	}
	...
	if err := NewPodListingReconciler(mgr, mirrorCluster); err != nil {
		panic(err)
	}
