...
// Integration tests: Docker (or another testcontainers backend) must be available.
// MongoDB image: mongodb-community-server 8.x (SCRAM-SHA-256 keyfile paths only; no legacy server for SCRAM-SHA-1).
// Default `go test` skips this file. Run manually when changing the validator or MongoDB wiring, e.g.:
//
//	cd cmd/connectivity-validator && go test -tags=integration ./tests/ -count=1 -v
//
// CI can opt in with the same -tags=integration on a job that provides Docker.
...
const (
	...
	mongodAuthMechanismsParam = "authenticationMechanisms=SCRAM-SHA-256"
)
...
func startMongodWithKeyfile(ctx context.Context, t *testing.T) string {
	...
	startCmd := keyfileSetupCmd + ` && exec mongod --setParameter ` + mongodAuthMechanismsParam + ` --auth --keyFile /tmp/mongo-keyfile --bind_ip_all`
...
func startMongodWithTLS(ctx context.Context, t *testing.T) (addr, caPath string) {
	...
	startCmd := keyfileSetupCmd + ` && exec mongod --setParameter ` + mongodAuthMechanismsParam + ` --tlsMode requireTLS --tlsCertificateKeyFile /certs/srv.pem --tlsCAFile /certs/ca.crt --tlsAllowConnectionsWithoutCertificates --auth --keyFile /tmp/mongo-keyfile --bind_ip_all`
	req := testcontainers.ContainerRequest{
		Image:        mongoImage,
		Cmd:          []string{"/bin/bash", "-c", startCmd},
		ExposedPorts: []string{"27017/tcp"},
...
func TestValidate_TwoMembers_BothReachable(t *testing.T) {
	...
	connStr := fmt.Sprintf("mongodb://%s/?directConnection=true&serverSelectionTimeoutMS=5000", addr1)
