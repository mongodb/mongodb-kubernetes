...
type MongoDBUser struct {
	...
	// CustomData is the user's MongoDB customData document, surfaced in the
	// automation config as auth.usersWanted[].customData and persisted onto the
	// mongod user. The operator uses it to stamp ownership (e.g. managedBy) so
	// Ops Manager can tell an operator-managed user apart from one it created.
...
const (
	// agentAuthSecretSuffix is the suffix AuthSecretName appends to an mdb name.
	agentAuthSecretSuffix = "-agent-auth-secret"

	// MonarchShipperPasswordKey is the data key under which the Monarch shipper
	// (mms-shipper) cleartext password is stored in `<mdbName>-monarch-shipper-secret`.
	MonarchShipperPasswordKey = "shipperPassword"
)

// MonarchShipperSecretName returns the name of the secret holding the Monarch
// shipper (mms-shipper) password for the given mdb name.
func MonarchShipperSecretName(mdbName string) string {
	return fmt.Sprintf("%s-monarch-shipper-secret", mdbName)
}

// ActiveMdbNameFromAgentAuthSecretRef derives the active cluster's mdb name from
// its agent auth secret name (`<active>-agent-auth-secret`) by stripping the
// `-agent-auth-secret` suffix. It is the inverse of AuthSecretName. Returns the
// input unchanged if the suffix is absent (defensive — callers always pass a
// SourceAgentAuthSecretRef.Name which carries the suffix by convention).
func ActiveMdbNameFromAgentAuthSecretRef(sourceAgentAuthSecretName string) string {
	return strings.TrimSuffix(sourceAgentAuthSecretName, agentAuthSecretSuffix)
}

// EnsureMonarchShipperPassword makes sure there is a cleartext password for the
// Monarch shipper user (mms-shipper) for an active cluster, stored in
// `<mdbName>-monarch-shipper-secret`. It mirrors EnsurePassword: read the secret
// and reuse the stored value when present, otherwise generate one and persist it.
// The password is returned so it can be added to the deployment auth (as the
// mms-shipper user's initPwd) and later embedded into the shipper's URIs.
//
// The operator owns this credential rather than relying on Ops Manager to inject
// it server-side: OM only surfaces shipperUser/shipperPwd on a later agent-API AC
// fetch, which caused a poll-fetch race. Generating it here lets the operator add
// the user to the same AC push as maintainedMonarchComponents. This relies on the
// OM-side idempotency guard (separate PR) that no-ops when the user already exists.
//
// Idempotent across reconciles.
func EnsureMonarchShipperPassword(ctx context.Context, k8sClient secret.GetUpdateCreator, namespace, mdbName string) (string, error) {
	secretName := MonarchShipperSecretName(mdbName)
	secretKey := client.ObjectKey{Name: secretName, Namespace: namespace}

	var password string
	if data, err := secret.ReadStringData(ctx, k8sClient, secretKey); err == nil {
		if val, ok := data[MonarchShipperPasswordKey]; ok && len(val) > 0 {
			password = val
		}
	}

	if password == "" {
		generated, err := generate.RandomFixedLengthStringOfSize(24)
		if err != nil {
			return "", err
		}
		password = generated
	...
	passwordSecret := secret.Builder().
		...
		SetStringMapToData(map[string]string{MonarchShipperPasswordKey: password}).
		Build()
	if err := secret.CreateOrUpdateIfNeeded(ctx, k8sClient, passwordSecret); err != nil {
		return "", fmt.Errorf("failed to update Monarch shipper password secret %s/%s: %w", namespace, secretName, err)
	}
	return password, nil
}

// SeedMonarchShipperSecretFrom copies the Monarch shipper password from the
// active cluster's shipper secret into the standby cluster's shipper secret
// (`<targetMdbName>-monarch-shipper-secret`). A standby must never generate its
// own shipper password — it is bootstrapped from the active's snapshot and must
// use the active's mms-shipper credential. Mirrors SeedAgentAuthSecretFrom.
//
// sourceSecretName is the active's shipper secret name
// (`<active>-monarch-shipper-secret`).
//
// Idempotent: no-op when the target already holds the source password.
func SeedMonarchShipperSecretFrom(ctx context.Context, k8sClient secret.GetUpdateCreator, namespace, sourceSecretName, targetMdbName string) error {
	sourceKey := client.ObjectKey{Name: sourceSecretName, Namespace: namespace}
	sourceData, err := secret.ReadStringData(ctx, k8sClient, sourceKey)
	if err != nil {
		return fmt.Errorf("failed to read source Monarch shipper secret %s/%s: %w", namespace, sourceSecretName, err)
	}
	password, ok := sourceData[MonarchShipperPasswordKey]
	if !ok || len(password) == 0 {
		return fmt.Errorf("source Monarch shipper secret %s/%s is missing key %q", namespace, sourceSecretName, MonarchShipperPasswordKey)
	}

	targetName := MonarchShipperSecretName(targetMdbName)
	targetKey := client.ObjectKey{Name: targetName, Namespace: namespace}
	if existing, err := secret.ReadStringData(ctx, k8sClient, targetKey); err == nil {
		if existing[MonarchShipperPasswordKey] == password {
			return nil
		}
	}

	targetSecret := secret.Builder().
		SetName(targetName).
		SetNamespace(namespace).
		SetStringMapToData(map[string]string{MonarchShipperPasswordKey: password}).
		Build()
	if err := secret.CreateOrUpdateIfNeeded(ctx, k8sClient, targetSecret); err != nil {
		return fmt.Errorf("failed to seed target Monarch shipper secret %s/%s: %w", namespace, targetName, err)
	}
	return nil
}

// SeedAgentAuthSecretFrom copies the automation-agent-password from a source
// agent auth secret into the target cluster's agent auth secret
// (`<targetMdbName>-agent-auth-secret`), so the target's EnsurePassword adopts
// the source's password instead of generating its own.
//
// This is required for a Monarch standby: it is bootstrapped via FCBIS from the
// active's snapshot, which carries the active cluster's mms-automation-agent
// SCRAM credential. The standby's agent must authenticate with the active's
// automation-agent-password, so this must run before the standby's first
// automation-config push (EnsurePassword reads the target secret then).
//
// Idempotent: no-op when the target already holds the source password.
func SeedAgentAuthSecretFrom(ctx context.Context, k8sClient secret.GetUpdateCreator, namespace, sourceSecretName, targetMdbName string) error {
	sourceKey := client.ObjectKey{Name: sourceSecretName, Namespace: namespace}
	sourceData, err := secret.ReadStringData(ctx, k8sClient, sourceKey)
	if err != nil {
		return fmt.Errorf("failed to read source agent auth secret %s/%s: %w", namespace, sourceSecretName, err)
	}
	password, ok := sourceData[constants.AutomationAgentAuthSecretKey]
	if !ok || len(password) == 0 {
		return fmt.Errorf("source agent auth secret %s/%s is missing key %q", namespace, sourceSecretName, constants.AutomationAgentAuthSecretKey)
	}

	targetName := AuthSecretName(targetMdbName)
	targetKey := client.ObjectKey{Name: targetName, Namespace: namespace}
	if existing, err := secret.ReadStringData(ctx, k8sClient, targetKey); err == nil {
		if existing[constants.AutomationAgentAuthSecretKey] == password {
			return nil
		}
	}
	...
	targetSecret := secret.Builder().
		SetName(targetName).
		SetNamespace(namespace).
		SetStringMapToData(map[string]string{constants.AutomationAgentAuthSecretKey: password}).
		Build()
	if err := secret.CreateOrUpdateIfNeeded(ctx, k8sClient, targetSecret); err != nil {
		return fmt.Errorf("failed to seed target agent auth secret %s/%s: %w", namespace, targetName, err)
	}
	return nil
}
