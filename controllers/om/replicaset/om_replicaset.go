...
func BuildFromStatefulSet(mongoDBImage string, set appsv1.StatefulSet, dbSpec mdbv1.DbSpec, fcv string) om.ReplicaSetWithProcesses {
	return BuildFromStatefulSetWithReplicas(mongoDBImage, set, dbSpec, int(*set.Spec.Replicas), fcv)
}
...
// parameter.
func BuildFromStatefulSetWithReplicas(mongoDBImage string, set appsv1.StatefulSet, dbSpec mdbv1.DbSpec, replicas int, fcv string) om.ReplicaSetWithProcesses {
	members := process.CreateMongodProcessesWithLimit(mongoDBImage, set, dbSpec, replicas, fcv)
