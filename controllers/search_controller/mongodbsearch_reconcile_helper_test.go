...
func TestMongoDBSearchReconcileHelper_ValidateSearchSource(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			db := construct.NewSearchSourceDBResourceFromMongoDBCommunity(&c.mdbc)
...
func TestMongoDBSearchReconcileHelper_ValidateSingleMongoDBSearchForSearchSource(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			helper := NewMongoDBSearchReconcileHelper(kubernetesClient.NewClient(clientBuilder.Build()), mdbSearch, construct.NewSearchSourceDBResourceFromMongoDBCommunity(mdbc), OperatorSearchConfig{})
