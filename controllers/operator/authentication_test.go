...
func TestUpdateOmAuthentication_NoAuthenticationEnabled(t *testing.T) {
	...
	r.updateOmAuthentication(ctx, conn, processNames, rs, "", "", "", false, zaptest.NewLogger(t).Sugar())
...
func TestUpdateOmAuthentication_EnableX509_TlsNotEnabled(t *testing.T) {
	...
	kubeClient, omConnectionFactory := mock.NewDefaultFakeClient(rs)
	r := newReplicaSetReconciler(ctx, kubeClient, nil, "", "", false, false, omConnectionFactory.GetConnectionFunc)
	status, isMultiStageReconciliation := r.updateOmAuthentication(ctx, conn, []string{"my-rs-0", "my-rs-1", "my-rs-2"}, rs, "", "", "", false, zaptest.NewLogger(t).Sugar())
...
func TestUpdateOmAuthentication_EnableX509_WithTlsAlreadyEnabled(t *testing.T) {
	...
	r := newReplicaSetReconciler(ctx, kubeClient, nil, "", "", false, false, omConnectionFactory.GetConnectionFunc)
	status, isMultiStageReconciliation := r.updateOmAuthentication(ctx, omConnectionFactory.GetConnection(), []string{"my-rs-0", "my-rs-1", "my-rs-2"}, rs, "", "", "", false, zaptest.NewLogger(t).Sugar())
...
func TestUpdateOmAuthentication_AuthenticationIsNotConfigured_IfAuthIsNotSet(t *testing.T) {
	...
	status, _ := r.updateOmAuthentication(ctx, omConnectionFactory.GetConnection(), []string{"my-rs-0", "my-rs-1", "my-rs-2"}, rs, "", "", "", false, zaptest.NewLogger(t).Sugar())
