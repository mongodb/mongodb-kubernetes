...
func VerifyTLSSecretForStatefulSet(secretResource corev1.Secret, opts Options) (string, error) {
	...
	crt, ok := secretResource.Data["tls.crt"]
	if !ok {
		return "", fmt.Errorf("Secret %s does not containa a tls.crt key", secretResource.Name)
	}

	key, ok := secretResource.Data["tls.key"]
	if !ok {
		return "", fmt.Errorf("Secret %s does not containa a tls.key key", secretResource.Name)
	}
...
func VerifyAndEnsureCertificatesForStatefulSet(secretGetterCreator secret.GetUpdateCreateDeleter, secretName string, opts Options, log *zap.SugaredLogger) (error, bool) {
	s, err := secretGetterCreator.GetSecret(kube.ObjectKey(opts.Namespace, secretName))
	if err != nil {
		return err, true
	}
	...
	if s.Type == corev1.SecretTypeTLS {
		data, err := VerifyTLSSecretForStatefulSet(s, opts)
		if err != nil {
			return err, true
		}

		secretHash := enterprisepem.ReadHashFromSecret(secretGetterCreator, opts.Namespace, secretName, log)
		return CreatePEMSecret(secretGetterCreator, kube.ObjectKey(opts.Namespace, secretName), map[string]string{secretHash: data}, opts.OwnerReference, log), true
...
// If the secret is of type kubernetes.io/tls, it creates a new secret containing the concatenation fo the tls.crt and tls.key fields
func VerifyAndEnsureClientCertificatesForAgentsAndTLSType(secretGetCreateDeleter secret.GetUpdateCreateDeleter, secret types.NamespacedName, log *zap.SugaredLogger) (error, bool) {

	s, err := secretGetCreateDeleter.GetSecret(secret)
	if err != nil {
		return err, true
	}

	if s.Type == corev1.SecretTypeTLS {
		data, err := VerifyTLSSecretForStatefulSet(s, Options{Replicas: 0})
		if err != nil {
			return err, false
		}
		...
		return CreatePEMSecret(secretGetCreateDeleter, secret, dataMap, []metav1.OwnerReference{}, log), true
...
func EnsureSSLCertsForStatefulSet(client kubernetesClient.Client, ms mdbv1.Security, opts Options, log *zap.SugaredLogger) (workflow.Status, bool) {
	...
	return ValidateSelfManagedSSLCertsForStatefulSet(client, secretName, opts, log)
...
// It also returns a boolean value indicating whether or no the new tls design must be used
func ValidateSelfManagedSSLCertsForStatefulSet(client kubernetesClient.Client, secretName string, opts Options, log *zap.SugaredLogger) (workflow.Status, bool) {
	...
	err, newTlsDesign := VerifyAndEnsureCertificatesForStatefulSet(client, secretName, opts, log)
	if err != nil {
