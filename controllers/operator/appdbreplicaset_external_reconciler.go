...
func (e *ReconcileExternalAppDBReplicaSet) ensureAppDBStatefulSetOwnership(ctx context.Context, opsManager *omv1.MongoDBOpsManager, clusterList []appDBClusterItem) error {
	...
	// Otherwise every expected AppDB StatefulSet must exist;
	// a missing one means its cluster number does not match the external reference's topology.
	for _, clusterItem := range clusterList {
	...
	for _, clusterItem := range clusterList {
		...
		if sts.Labels[util.MongoDBOpsManagerResourceOwnerLabel] != opsManager.GetOwnerLabels()[util.MongoDBOpsManagerResourceOwnerLabel] {
			continue
		}

		if err := requestAppDBForwardMigration(ctx, clusterItem.client, opsManager, sts); err != nil {
...
func requestAppDBForwardMigration(ctx context.Context, c client.Client, opsManager *omv1.MongoDBOpsManager, sts appsv1.StatefulSet) error {
	filteredOwnerRefs := make([]metav1.OwnerReference, 0, len(sts.OwnerReferences))
	...
	sts.OwnerReferences = filteredOwnerRefs
