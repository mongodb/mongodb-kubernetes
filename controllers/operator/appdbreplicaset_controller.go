	...
	builder := automationconfig.NewBuilder().
		...
		AddModifications(func(automationConfig *automationconfig.AutomationConfig) {
			if construct.ShouldEnableMonitoring(podVars) {
				configureMonitoring(automationConfig, log, rs.GetSecurity().IsTLSEnabled(), podVars.ProjectID, podVars.AgentAPIKey)
...
func configureMonitoring(ac *automationconfig.AutomationConfig, log *zap.SugaredLogger, tls bool,  projectID string, agentAPIkey string) {
	...
	for _, p := range ac.Processes {
		...
		if foundIdx == -1 {
			...
			mv.AdditionalParams["mmsGroupId"] = projectID
			mv.AdditionalParams["mmsApiKey"] = projectID
			...
			monitoringVersions[foundIdx].AdditionalParams["mmsGroupId"] = projectID
			monitoringVersions[foundIdx].AdditionalParams["mmsApiKey"] = projectID
		}
	}
