...
// ensureTLSSecretAndCreatePEMIfNeeded checks that the needed TLS secrets are present and creates the concatenated PEM secret.
// The secret must be of type kubernetes.io/tls; it reads the tls.crt and tls.key entries and generates a new secret
...
func (r *ReconcileAppDbReplicaSet) ensureTLSSecretAndCreatePEMIfNeeded(ctx context.Context, om *omv1.MongoDBOpsManager, log *zap.SugaredLogger) workflow.Status {
	...
	if vault.IsVaultSecretBackend() {
		...
		if s.Type != corev1.SecretTypeTLS {
			return workflow.Failed(xerrors.Errorf("secret %q must be of type kubernetes.io/tls, got %q", secretName, s.Type))
		}
		needToCreatePEM = true
		secretData = s.Data
	}
