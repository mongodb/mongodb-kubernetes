...
func TestMDBMultiAppDBRoleSecrets_ClaimedByCR(t *testing.T) {
	// forward migration: the secrets pre-exist (created by the internal AppDB); the CR must claim
	// them so its eventual deletion garbage-collects them together with the StatefulSets
	ctx := context.Background()
...
func TestMDBMultiReconcile_ReleasedAppDBRoleDoesNotReclaimSecrets(t *testing.T) {
	// regression: the claim must run only after every AppDB cluster passed its ownership gate - a
	// CR re-claiming the Ops Manager-owned handover secrets while awaiting release would
	// garbage-collect them on its deletion, rotating the keyfile under a running internal AppDB
	ctx := context.Background()
...
func TestMDBMultiAppDBAdoptionGate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "own ref plus no label is unowned",
			role: "AppDB",
			setup: func(t *testing.T, mrs *mdbmulti.MongoDBMultiCluster, reconciler *ReconcileMongoDbMultiReplicaSet, clusterMap map[string]client.Client) {
