		...
		Containers: []corev1.Container{
			{
				...
				//
				// Token notes (spdlog):
				//   %Y-%m-%dT%H:%M:%S.%e%z → ISO 8601 with ms and ±HH:MM
				//     offset (matches mongod/mongot timestamp shape so any
				//     downstream tooling can sort cross-layer by wall clock).
				//   %g:%# → "<source-path>:<line>". The field is named ``loc``
				//     (not ``source``) on purpose: lnav 0.13's built-in JSON
				//     graylog demuxer keys on ``source`` + ``message`` and
				//     would otherwise split this log into per-file pipers
				//     before any format detection runs.
				Args: []string{
