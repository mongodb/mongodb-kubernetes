...
func (r *OpsManagerReconciler) Reconcile(ctx context.Context, request reconcile.Request) (res reconcile.Result, e error) {
	...
	appDbReconciler, appDBReconcilerErr := r.createAppDBReconcile(ctx, opsManager, memberClusterEntries, log)
...
func (r *OpsManagerReconciler) createAppDBReconcile(ctx context.Context, opsManager *omv1.MongoDBOpsManager, memberClusterEntries map[string]multicluster.Entry, log *zap.SugaredLogger) (AppDBReconciler, error) {
	if opsManager.IsInternalAppDB() {
		return r.createNewAppDBReconciler(ctx, opsManager, memberClusterEntries, log)
	}
