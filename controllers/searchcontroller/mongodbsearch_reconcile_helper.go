...
func buildProxyService(search *searchv1.MongoDBSearch, unit reconcileUnit) corev1.Service {
	...
	// AppProtocol=tcp makes Istio (and meshes honouring the K8s appProtocol field)
	// skip protocol sniffing, overriding the "grpc" port-name rule that would apply
	// HTTP/2 L7 processing to mongod's raw-TCP gRPC handshake. Without it, sidecar-
	// injected namespaces drop mongod→Envoy traffic and createSearchIndexes returns
	// code 125. The port name stays "grpc" for name-based lookups.
