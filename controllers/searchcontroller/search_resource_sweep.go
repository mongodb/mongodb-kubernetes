...
// given MongoDBSearch: cached List per descriptor, selected by the search
// identity labels, then delete. Failures are collected per object and never
// abort the sweep. clusterName is log context only. Returns whether any owned
// candidate was found, deleted or not — callers waiting for a resource to be
// fully gone gate on it.
