...
environment, using 2 GCP clusters and Istio.

The end result will be a 5 member Replica Set, deployed across the 2 clusters,
with 3 members in `cluster1` and 2 members in `cluster2`. The Pods will be
...
is 5; the Services will have to be created in both clusters.
...
## Set default network for `cluster2`
...
After completing these steps, we should have a configured Istio mesh in 2 GCP
clusters.
...
``` shell
kubectl -n mdb apply -f svc-5-pods.yaml --context cluster1
kubectl -n mdb apply -f svc-5-pods.yaml --context cluster2
```

This will create 10 Services, 5 on each cluster.
...
``` shell
kubectl -n mdb apply -f sts-headless-cluster1.yaml --context cluster1
kubectl -n mdb apply -f sts-headless-cluster2.yaml --context cluster2
```
