...
- **Q3 (operator-managed mongod) — out of MVP per 2026-05-04 scope clarification; not currently planned.** If pursued post-MVP, it would flow through the operator's existing AC writer: the operator already manages automation config end-to-end, so per-cluster `mongotHost` would plumb through whatever pipeline operator-managed mongods use today for `setParameter` overrides — no new schema and no new mechanism required.
...
- **MVP scope clarification (2026-05-04): externally-managed mongod ONLY; operator-managed mongod is out of MVP entirely.** Direct user direction: "We will only support externally managed mongod in MVP and no operator managed mongod." This removes from MVP consideration: Phase 4 (Q1-RS-MC managed), Phase 5 (Q1-Sh-MC managed), and Q3 (operator-managed mongod handling per-cluster `mongotHost` via the operator's AC writer). Phase 2 (Q2-RS-MC = external/customer-managed) remains the *only* MVP delivery target. Phase 3 (Q2-Sh-MC = external/customer-managed sharded) is post-MVP but remains the only sharded path that could re-enter MVP under the externally-managed-only constraint if scope ever expands; Phase 4/5 cannot. Treat all Phase 4/5/Q3 design content as post-MVP design parking only — see [`2026-05-03-mc-mvp-later-phases.md`](./2026-05-03-mc-mvp-later-phases.md).
