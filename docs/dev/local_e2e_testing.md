...
- [Local development and E2E testing](#local-development-and-e2e-testing)
  ...
  * [Multi-cluster development](#multi-cluster-development)
    ...
    + [Running tests against Evergreen host](#running-tests-against-evergreen-host)
      ...
      - [All steps combined to run test locally](#all-steps-combined-to-run-test-locally)
      - [Other uses of `evg_host.sh` script](#other-uses-of--evg-hostsh--script)
      - [Known issues](#known-issues)
    + [Running tests against our static multi-cluster](#running-tests-against-our-static-multi-cluster)
      - [Context variables](#context-variables-1)
      - [All steps combined to run test locally](#all-steps-combined-to-run-test-locally-1)
...
go run main.go -watch-resource=mongodb -watch-resource=opsmanagers -watch-resource=mongodbusers -watch-resource=mongodbmulticluster -cluster-names=kind-e2e-cluster-1,kind-e2e-cluster-2,kind-e2e-cluster-3
```
...
```
-watch-resource=mongodb -watch-resource=opsmanagers -watch-resource=mongodbusers -watch-resource=mongodbmulticluster -cluster-names=kind-e2e-cluster-1,kind-e2e-cluster-2,kind-e2e-cluster-3
```
...
### Running tests against our static multi-cluster

#### Context variables

Use and modify context file from `scripts/dev/samples/multi`
...
```bash
source ~/.operator-dev/context.operator.export.env
go run main.go -watch-resource=mongodb -watch-resource=opsmanagers -watch-resource=mongodbusers -watch-resource=mongodbmulticluster -cluster-names=e2e.cluster1.mongokubernetes.com,e2e.cluster2.mongokubernetes.com,e2e.cluster3.mongokubernetes.com
```
...
```
-watch-resource=mongodb -watch-resource=opsmanagers -watch-resource=mongodbusers -watch-resource=mongodbmulticluster -cluster-names=e2e.cluster1.mongokubernetes.com,e2e.cluster2.mongokubernetes.com,e2e.cluster3.mongokubernetes.com
```
...
* `cp multi context to ~/.operator-dev`
  * `make updates to the env vars marked with <UPDATE ME>`
* `source ~/.operator-dev/contexts/multi`
* `./scripts/dev/prepare_local_e2e_run.sh` - This script creates all the necessary files our test automation relies on.
...
In the operator command line arguments do not pass mongodbmulticluster watch-resource as it would switch the operator to multi-cluster mode.
```
