...
| 12 | Unified member RBAC names | CLOUDP-400899 | done | `--member-cluster` **removed from `generate-member-resources`**: member-side resources render under fixed `mck-member-*` names (credentials `mck-member-sa`/`mck-member-token`; roles `mck-member-role-base`/`mck-member-role-multicluster`/`mck-member-pvc-resize` + bindings; telemetry `mck-member-cluster-telemetry` + binding; workload SAs `mck-member-{appdb,database-pods,ops-manager}`) instead of `mck-member-<cluster>-*`, so **one render applies unchanged to every member cluster**. `memberCluster.name` chart value dropped (it was never in values.yaml; the `required` template guards deleted). Operator SA derivation simplified to `workloadServiceAccount.Name(baseInstall)` — member mode returns the fixed names, no per-cluster input. (Originally staged to keep the slice-7 clusterName→CR-name registry unused-but-present; slice 9 landed first and already removed it, so the rebase simply drops it.) `generate-member-registration` **keeps `--member-cluster`** (user decision): its credential Secret (`mck-credential-<name>`) + `MemberCluster` CR land on the **central** cluster and must stay per-cluster to avoid cross-cluster conflicts; only its token-Secret lookup on the member cluster became the fixed `mck-member-token`. E2E: recover-clusterwide cleanup list + `om_ops_manager_pod_spec` SA expectations unified (only None-ness of the member-cluster name still discriminates single- vs multi-cluster there); gitops samples regenerated from the CLI (render-once output). Base render byte-identical (`helm template` diff-verified across three value combos); OLM provably unaffected (`config/rbac` is a base-mode render only). (2026-09-03: the unified `mck-member-*` roles now render as one namespaced Role per binding namespace in narrowed mode — ClusterRoles only under `--operator-cluster-scoped`.) Changelog: none — the `20260831_breaking_unified_member_cluster_rbac` entry was dropped again (the feature branch is unreleased, so the naming change needs no migration note); the follow-up layer adds `20260903_breaking_multi_namespace_install_namespaced_rbac` for the base-chart multi-namespace change, which **is** released 1.x behaviour. **(Superseded in part by slice 14: the credential render and the fixed `mck-member-token` lookup are gone — credentials are user-provisioned.)** |
| 14 | User-provided member-cluster credentials | CLOUDP-400899 | in progress | Day-2 configuration model: the base install stays single-cluster-simple; the operator's member-cluster credentials become the user's first day-2 step. `generate-member-resources` **no longer renders** the member SA `mck-member-sa` / token Secret `mck-member-token` (deleted from `member-cluster-rbac.yaml`, which now holds only `mck-member-role-multicluster` + binding); both commands gain **required `--member-cluster-service-account`** — resources binds every rendered binding to it (new plugin-internal `memberCluster.serviceAccount` value, consumed by the four binding-subject sites in `member-cluster-rbac`/`operator-roles-base`/`-pvc-resize`/`-telemetry`), and registration discovers the token Secret by listing `kubernetes.io/service-account-token` Secrets filtered on the `kubernetes.io/service-account.name` annotation (0 matches → immediate error; >1 → ambiguity error; 1 → the existing 1-minute wait now covers only `token`/`ca.crt` population; the SA itself is GET first for a clear not-found error). `resourcenames.MemberClusterTokenSecretName()` deleted (sole consumer was the CLI). **Order per member cluster: credentials → RBAC → registration** (registration-first was rejected: it hands the operator a permissionless credential for a window). Credentials manifest ships as a static sample (`public/samples/multi-cluster-cli-gitops/resources/credentials.yaml`) and inline in the CI-executed snippets (`12_0110`/`13_0110`/`ra-02_0220`); harness `configure_multi_cluster_members` prepends the credentials apply. SA name `mck-member-sa` used throughout (arbitrary in principle). Optional-feature pattern formalised for future flags (e.g. `--operator-auto-tls`): a discrete `--operator-*` render flag + matching `OperatorConfig` knob; **no OperatorConfig API change in this slice** (`spec.telemetry.mode` already exists). Changelog: no new entry — `20260715`/`20260716`/`20260722` updated in place (unreleased feature-branch surface). Workload RBAC (`database-roles.yaml`, `mck-member-{appdb,database-pods,ops-manager}`) unchanged — still CLI-rendered. |

**Dependencies:** 3 → {1, 2}; 4 → {1, 3}; 5 → {1, 2}; 6 → 5; 7 → 3 (needs multi-cluster reconcile working; can land any time after); 8 → {5, 7, 10} (runs on the settled, E2E-covered shape); 9 → 3 (makes the slice-3 restart-based watch reactive; resolves the slice-5 local-operator caveat); 10 → 7 (settles the member-side template shape slice 8 dedups); 11 → 10 (renames the flags slice 10 introduced); 12 → {10, 11} (unifies the names those slices settled); 13 → {8, 12, 14} (simplifies the template shape those slices settled); 14 → 12 (revises the unified names: credentials leave the render).
...
- **The snippets deliberately do not wait for the operator to react to registration.** Membership
  ...
  (`_wait_for_member_sa_token` deleted). (Since slice 14 the Secret is user-provisioned and
  discovered via the `kubernetes.io/service-account.name` annotation; the wait covers only
  data-key population, never existence.)
