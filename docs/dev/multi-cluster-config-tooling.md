...
| 8 | RBAC de-duplication | CLOUDP-400899 | todo | Single source of truth for the operator's shared workload rules (services/secrets/configmaps/statefulsets/deployments/pods) so extending a permission is one edit, not two. Aim for: base role = shared + central-only (CRDs/operatorconfigs); member role = shared + member-only rules (`serviceaccounts get` for the rbac-version self-read — the telemetry extras were split out in slice 10 into the dual-mode `operator-roles-telemetry.yaml`). Mechanism left open (shared partial, restructured/parameterised template, generating member from the same source, …). Deferred deliberately — see below. Note: `database-roles.yaml` is already dual-mode since slice 7 (workload RBAC has one template serving base install and member render); the remaining dedup is `member-cluster-rbac.yaml` vs `operator-roles-base.yaml` namespaced workload rules. |
...
- the member role's cluster extras (`nodes`, `kube-system` namespace get, `/version`, `namespaces list/watch`) were only *effectively granted* in the `*` render (the lone ClusterRoleBinding — in the multi-namespace render they sat in the ClusterRole bound by per-namespace RoleBindings, where they were inert). MCK 1.x granted the telemetry extras (`nodes`/`kube-system`/`/version` — consumed by `pkg/telemetry/cluster.go` on every member cluster, fail-soft to `Unknown` when denied) via a **separate telemetry ClusterRoleBinding created regardless of scope** (`--create-telemetry-roles`, default true). The 2.x narrowed renders quietly dropped that parity — until **slice 10 restored it**: `operator-roles-telemetry.yaml` is now dual-mode, so the member render emits a scope-independent telemetry ClusterRole + ClusterRoleBinding (`mck-member-<c>-cluster-telemetry[-binding]`) gated by `--create-telemetry-roles`, and the scope-conditional extras block in `member-cluster-rbac.yaml` is deleted (including the dead `namespaces list/watch` rule and the wrong `nodes get` verb). The member role is namespaced-rules-only in every mode, so slice 8's remaining scope is just the namespaced workload rules.
