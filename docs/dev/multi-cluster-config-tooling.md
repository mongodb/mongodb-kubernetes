...
| 4 | RBAC validation | CLOUDP-400899 | cancelled for the initial 2.x release (partially salvaged) | Was to add an `RBACValid` condition on `MemberCluster.status` by probing the `mongodb.com/rbac-version` annotation on the member SA; the design settled 2026-08-21 as **status-only** — no reconciliation gate, best-effort MCK 1.x behaviour preserved. On 2026-09-01 it was decided the initial 2.x release ships **without** the condition: draft PR #1507 (`iux-multi-cluster-rbac-validation`) was repurposed to remove the scaffolding (the pre-landed `RBAC Valid` printcolumn and the `serviceaccounts get` self-read rule were removed again; the `mongodb.com/rbac-version` annotation keeps being rendered so validation can be reintroduced later without re-annotating member RBAC). **Salvaged from the slice:** credential-rotation pickup (the membercluster reconciler requeues every 60s and rebuilds a cluster's entry when the credential Secret's kubeconfig hash changes) and the kubeconfig-key writer/reader constant. A `MemberCluster` condition is still expected to return later (e.g. duplicate clusterName signalling — see Follow-ups). |
...
| 12 | Unified member RBAC names | CLOUDP-400899 | done | `--member-cluster` **removed from `generate-member-resources`**: member-side resources render under fixed `mck-member-*` names (credentials `mck-member-sa`/`mck-member-token`; roles `mck-member-role-base`/`mck-member-role-multicluster`/`mck-member-pvc-resize` + bindings; telemetry `mck-member-cluster-telemetry` + binding; workload SAs `mck-member-{appdb,database-pods,ops-manager}`) instead of `mck-member-<cluster>-*`, so **one render applies unchanged to every member cluster**. `memberCluster.name` chart value dropped (it was never in values.yaml; the `required` template guards deleted). Operator SA derivation simplified to `workloadServiceAccount.Name(baseInstall)` — member mode returns the fixed names, no per-cluster input. (Originally staged to keep the slice-7 clusterName→CR-name registry unused-but-present; slice 9 landed first and already removed it, so the rebase simply drops it.) `generate-member-registration` **keeps `--member-cluster`** (user decision): its credential Secret (`mck-credential-<name>`) + `MemberCluster` CR land on the **central** cluster and must stay per-cluster to avoid cross-cluster conflicts; only its token-Secret lookup on the member cluster became the fixed `mck-member-token`. E2E: recover-clusterwide cleanup list + `om_ops_manager_pod_spec` SA expectations unified (only None-ness of the member-cluster name still discriminates single- vs multi-cluster there); gitops samples regenerated from the CLI (render-once output). Base render byte-identical (`helm template` diff-verified across three value combos); OLM provably unaffected (`config/rbac` is a base-mode render only). (2026-09-03: the unified `mck-member-*` roles now render as one namespaced Role per binding namespace in narrowed mode — ClusterRoles only under `--operator-cluster-scoped`.) Changelog: none — the `20260831_breaking_unified_member_cluster_rbac` entry was dropped again (the feature branch is unreleased, so the naming change needs no migration note); the follow-up layer adds `20260903_breaking_multi_namespace_install_namespaced_rbac` for the base-chart multi-namespace change, which **is** released 1.x behaviour. **(Superseded in part by slice 14: the fixed `mck-member-token` lookup is gone — registration discovers the token Secret by annotation; the credential render stays as the default flow.)** |
| 13 | RBAC template simplification | CLOUDP-400899 | proposed | Simplify the dual-mode RBAC templates (`operator-roles-base`, `operator-roles-pvc-resize`, `member-cluster-rbac`, `database-roles`) now that their shape has settled (post slices 8/10/12 + the namespaced-RBAC change in #1554). Candidates: **(1)** drop the member-mode `fail "*"` backstops — `memberCluster.*` is plugin-internal and only the CLI renders it, and the CLI already validates (`normalizeWorkloadNamespaces`); `TestRender_RejectsWildcard` goes with them; valid only while direct `helm template` of member mode stays unsupported. **(2)** render the `mongodb.com/rbac-version` annotation unconditionally instead of `if $memberMode` (harmless on the base install; accepted: base render no longer byte-identical). **(3)** extract the central-only rules (`mongodbcommunity`, `mongodb.com`, `ai.mongodb.com`, `operatorconfigs`/`memberclusters`, currently `if not $memberMode` in `operator-roles-base.yaml`) into a separate base-only role/template so the shared template carries no mode gating (NB: changes the base render's object structure — OLM/`config/rbac` impact to assess). **(4)** revisit the slice-8 deferred decision: the binding mechanics (scope computation + per-namespace binding loop) are now quadruplicated; a `define` helper may pay off once the rules themselves no longer branch on mode. |
| 14 | Optional user-provided member-cluster credentials | CLOUDP-400899 | in progress | Both commands gain **optional `--member-cluster-service-account`**; flag presence carries the semantics. **Unset (default):** `generate-member-resources` renders the member SA `mck-member-sa` + token Secret as before (the blocks are gated on the new plugin-internal `memberCluster.createCredentials`), and registration discovers the default SA's token. **Set (BYO/GitOps):** the render skips the credentials and every member-mode binding subject uses the given SA (new plugin-internal `memberCluster.serviceAccount` value, consumed by the four binding-subject sites in `member-cluster-rbac`/`operator-roles-base`/`-pvc-resize`/`-telemetry`); registration discovers that SA's token. Registration has **one code path**: annotation-based discovery — list `kubernetes.io/service-account-token` Secrets filtered on `kubernetes.io/service-account.name` (0 matches → immediate error; >1 → ambiguity error naming candidates; 1 → the existing 1-minute wait now covers only `token`/`ca.crt` population; the SA is GET first for a clear not-found error) — with the resolved name (`mck-member-sa` default, constant in `pkg/resourcenames`). `resourcenames.MemberClusterTokenSecretName()` deleted (registration no longer looks up a fixed Secret name — the slice-12 fixed lookup is superseded; the credential *render* stays). RFC 1123 validation when the flag is set. E2E harness + CI snippets (`12_0110`/`13_0110`/`ra-02_0220`) exercise the **default flow** (no harness change vs pre-slice); the GitOps sample is the BYO showcase (static `resources/credentials.yaml` + flag-set regen in `generate_files.sh` + README). BYO coverage is unit-only (user decision; the paths differ only in which SA name flows through). Optional-feature pattern formalised for future flags (e.g. `--operator-auto-tls`): a discrete `--operator-*` render flag + matching `OperatorConfig` knob; **no OperatorConfig API change in this slice** (`spec.telemetry.mode` already exists). Changelog: no new entry — `20260715`/`20260716`/`20260722` updated in place (unreleased feature-branch surface). Workload RBAC (`database-roles.yaml`, `mck-member-{appdb,database-pods,ops-manager}`) unchanged — still CLI-rendered. |

**Dependencies:** 3 → {1, 2}; 4 → {1, 3}; 5 → {1, 2}; 6 → 5; 7 → 3 (needs multi-cluster reconcile working; can land any time after); 8 → {5, 7, 10} (runs on the settled, E2E-covered shape); 9 → 3 (makes the slice-3 restart-based watch reactive; resolves the slice-5 local-operator caveat); 10 → 7 (settles the member-side template shape slice 8 dedups); 11 → 10 (renames the flags slice 10 introduced); 12 → {10, 11} (unifies the names those slices settled); 13 → {8, 12, 14} (simplifies the template shape those slices settled); 14 → 12 (replaces the fixed token-Secret lookup with annotation discovery).
...
- **The snippets deliberately do not wait for the operator to react to registration.** Membership
  ...
  The wait that *is* load-bearing — for the member token Secret's data keys before
  `generate-member-registration` reads them — has since moved **into** the command
  ...
  (`_wait_for_member_sa_token` deleted). (Since slice 14 the Secret — rendered by default or
  pre-provisioned — is discovered via the `kubernetes.io/service-account.name` annotation;
  the wait covers only data-key population, never existence.)
...
- **Shared namespaced workload rules** (services/secrets/configmaps/statefulsets/deployments/pods)
  ...
  `mck-member-role-base`. Extending a shared permission is one edit again.
- **Multi-cluster-only rules** (`deletecollection` on the four kinds — used only by the
  MC `DeleteAllOf` cleanup paths, since owner references don't work across clusters) live in
  `mck-member-role-multicluster` in `member-cluster-rbac.yaml`, with why-comments (the
  `serviceaccounts get` rbac-version self-read rule was removed again when slice 4 was
  cancelled).
...
- **Member RBAC naming** `mck-member-*` (unified — the same fixed names on every member
  cluster, no per-cluster component), deliberately decoupled from
  `.Values.operator.name` (so it's unaffected by operator-name unification — see below).
...
- **Narrowed installs are purely namespaced** (2026-09-03): both the CLI member render and the
  base chart with an `operator.watchNamespace` list emit one Role per watched/workload namespace
  and no ClusterRoles for operator permissions, restoring MCK 1.x CLI parity (1.x
  `buildMemberEntityRole(namespace)` built one Role per namespace). ClusterRoles remain only for
  cluster-wide installs (`--operator-cluster-scoped` / `operator.watchNamespace="*"`) and the
  togglable features (telemetry, clustermongodbroles, webhook).

## Follow-ups

- **`MemberCluster.spec.clusterName` uniqueness.** Cross-object uniqueness cannot be enforced at the API level (no CEL for it; a webhook would race). Since slice 9 PR 2 the reconciler enforces it at runtime: first writer wins, a second CR claiming the same clusterName is refused (its reconcile errors and requeues loudly). Open design work, deliberately separate from the cancelled slice-4 `RBACValid` condition: surface the conflict on the losing CR's status (condition type/semantics TBD — e.g. `Accepted`) so users can see it without reading operator logs.
- **RBAC validation may return post-initial-release.** Slice 4's `RBACValid` design is parked, not rejected: `generate-member-resources` still renders the `mongodb.com/rbac-version` annotation, so the condition can be reintroduced later without re-annotating member RBAC.
- **Sample regeneration on version bumps.** Ensure that we generate public samples such as `public/samples/multi-cluster-cli-gitops/resources/rbac/cluster_scoped_member_cluster.yaml` correctly when bumping the operator version. It appears like `make precommit-full`/`make precommit` doesn't always (race?) generate the sample when the operator version is being bumped.
- **OperatorConfig no-restart.** The OperatorConfig watcher still restarts the operator on change. Follow-up: classify fields into hot-reloadable (timeouts, telemetry cadence, …) vs restart-bound (watch namespaces, watchedResources, webhook config) and introduce a live-config source for the former. File a ticket under the Operator Config TD.
- **Member RBAC clean-up UX (slice 14 follow-up).** The interim opt-in/opt-out flow is: delete the old member RBAC → re-render with different `--operator-*` flags → update `OperatorConfig`. Known tension: the render bundles workload RBAC (`mck-member-appdb` Role/RoleBinding, workload SAs) with operator permissions, so deleting the whole render disturbs running workloads (readiness impact). A proper clean-up flow should target **optional feature roles discretely** (they are already separate Role/Binding pairs, e.g. telemetry) rather than the whole render — possibly a dedicated CLI verb or a documented label-selector deletion.
