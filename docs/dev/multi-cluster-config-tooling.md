...
| 9 | No-restart `MemberCluster` reactivity (hot reload) | CLOUDP-400899 | in progress | Make membership changes reactive **without** restarting the operator — the "later slice" referenced from slice 3, which currently restarts the operator on `MemberCluster` add/spec-change/delete. **Mechanism decided**: hand-rolled `multicluster.Provider` + level-based `MemberCluster` reconciler (rejected: `multicluster-runtime` — a custom CRD-discovery provider is needed either way, real value requires an `mcbuilder`/`mcreconcile` migration of every MC controller, pre-1.0 API churn on critical wiring; our provider mirrors its `Provider.Get`/`Aware.Engage` seam so a later migration stays cheap). Two stacked PRs: **(1) `iux-multi-cluster-cluster-provider`** — zero-behaviour-change refactor: thread-safe `Provider` registry (`Entry{Cluster, Client, ResourceName}`) in `pkg/multicluster`, all 8 reconcilers + telemetry read per-reconcile snapshots instead of constructor-time maps, slice-7 resource-name registry (`pkg/resourcenames/member_cluster.go`) **removed** (mapping now flows on `Entry`/`multicluster.MemberCluster.ResourceName`); restart watcher still active. **(2) `iux-multi-cluster-hot-reload`** — the behavioural switch: reconciler replaces the watcher (per-entry cluster lifecycle, engage hooks for dynamic watches + enqueue-on-add), memberwatch hook-driven, E2E restart-waits replaced with no-restart assertions. This also resolves the **slice-5 local-dev caveat** (host-run operator exits on membership change). Tagged `TODO(m1kola): slice-9` in `docker/mongodb-kubernetes-tests/tests/conftest.py`. |
