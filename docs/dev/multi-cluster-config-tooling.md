...
| 6 | Clean break | CLOUDP-400899 | todo | Remove `setup`/`recovery`, the legacy discovery + fallback, and dead `common.go` RBAC/kubeconfig code. Also remove the Helm values the slice-5 migration stopped setting once nothing sets them: the `operator.createOperatorServiceAccount=false` override, `multiCluster.clusters`, and `multiCluster.kubeConfigSecretName` — plus the `kube-config-volume` mount they gate in `helm_chart/templates/operator.yaml:59-61,263-267`. |
