...
| 7 | Member-scoped workload ServiceAccounts | CLOUDP-400899 | todo | End-state so `generate-member-resources` output touches **nothing** from helm/OLM. Un-hardcode the workload pod SA names in the operator (`construct/appdb_construction.go:500`, `construct/opsmanager_construction.go:480`; database SA already per-CR overridable) so pods on member clusters run under member-scoped SAs; emit member-scoped workload RBAC instead of the interim fixed-name `database-roles.yaml`. Single-cluster keeps using the helm-install SAs. **Also split `_install_multi_cluster_operator`** (conftest.py, deferred from slice 6 PR 3): it still serves two flows keyed on the `configure_member_clusters` flag — the released baseline (legacy kubeconfig-Secret + member-list provisioning, plus `prepare_multi_cluster_namespaces` rendering the *released* chart's database-roles onto members — both permanent, since frozen pre-2.x baseline charts always need them) and the new path (`MemberCluster` CRs + credential Secrets). Not gated on this slice — the split can land any time — but natural to bundle here, since slice 7 already revisits every `prepare_multi_cluster_namespaces` call site (the direct new-path callers drop it as workload RBAC becomes member-scoped; the released-baseline call stays). |
