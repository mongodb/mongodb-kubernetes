...
| 4 | RBAC validation | CLOUDP-400899 | done | `RBACValid` condition on `MemberCluster.status` (True/False/**Unknown**), written by the slice-9 `membercluster.Reconciler` probing the `mongodb.com/rbac-version` annotation on the member SA `mck-member-<CR-name>-sa`; the SA namespace comes from the credential kubeconfig's context (`generate-member-registration` stamps it; **no fallback** — missing namespace is a surfaced `CredentialNamespaceMissing` error). **Open design question resolved to absent-entry**: `False` (SA missing, probe forbidden, annotation missing/mismatch, credential problems) removes the provider entry via the existing Delete+OnRemove path — zero workload-reconciler changes (every consumer already skips absent clusters), and no auto-failover on misconfiguration (memberwatch checker removed); `Unknown` (unreachable/timeout) keeps or builds the entry, preserving today's unreachable-cluster behaviour incl. failover. Expected version is ldflags-injected (`util.ExpectedMemberRBACVersion` from `release.json`'s `mongodbOperator`) in pipeline image builds **and** `make run` — not `util.OperatorVersion`, which is a SHA in CI patches; empty (bare `go run`/`go build`) → `True`/`ValidationDisabled`. Fixed **60s** re-check (decision: no OperatorConfig knob; possible post-2.0.0) drives RBAC-fix pickup, credential rotation (kubeconfig hash on the entry state), and late-Secret retry. `memberclusters/status` RBAC added to `operator-roles-base.yaml` (central-only) → config/rbac + OLM. `GenerationChangedPredicate` keeps status writes from re-triggering probes. E2E: harness-wide `RBACValid=True` wait in `configure_multi_cluster_members` (upgrade tests opt out at pre-upgrade registration) + dedicated negative test `multi_cluster_rbac_validation.py` (`TODO(m1kola)`: may be subsumed by the migration-path E2E). Version-coupling guard test: Chart.yaml == release.json (`helm_chart/embed_test.go`). Note: ensure that we generate public samples such as `public/samples/multi-cluster-cli-gitops/resources/rbac/cluster_scoped_member_cluster.yaml` correctly when bumping the operator version. It appears like `make precommit-full`/`make precommit` doesn't always (race?) generate the sample when the operator version is being bumped. |
...
- **`MemberCluster.spec.clusterName` uniqueness.** Cross-object uniqueness cannot be enforced at the API level (no CEL for it; a webhook would race). Since slice 9 PR 2 the reconciler enforces it at runtime: first writer wins, a second CR claiming the same clusterName is refused (its reconcile errors and requeues loudly). Open design work, deliberately separate from slice 4's `RBACValid`: surface the conflict on the losing CR's status (condition type/semantics TBD — e.g. `Accepted`) so users can see it without reading operator logs.
