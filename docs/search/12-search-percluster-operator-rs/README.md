...
To make the runbook match the operator version you're deploying (recommended, since `master` moves with operator development), add `--branch <tag>` to the clone below, using that version's release tag from the [releases page](https://github.com/mongodb/mongodb-kubernetes/releases), e.g. `--branch 1.10.0`.
...
| Automation agent | **108.0.13.8870** | required by MongoDB 8.2+; operator charts can default to LESS, so check yours and set `agent.version` on the ra-02 operator release if needed |
...
- **The customer already runs OM** (the common field reality): skip the deploy, since all this scenario needs is the org API key Secret and project ConfigMap, i.e. what `ra-06_0610` (last line below) creates.
...
You'll know it worked when the central operator's args no longer list `mongodbsearch`: `kubectl get deploy mongodb-kubernetes-operator-multi-cluster -n ${OPERATOR_NAMESPACE} -o yaml | grep watch-resource`. The `Running`/`Invalid` flap stops once the Search CRs exist.
...
`spec.source.external.tls.ca` requires a ConfigMap with a `ca.crt` key specifically, distinct from `ra-05`'s own `ca-issuer` ConfigMap, which only carries `ca-pem`/`mms-ca.crt`. This is a plain copy of `ra-05`'s existing CA cert content, with no local cert-manager involved.
...
**Why:** The presence check does a Secret `Get` for every listed name, including the CA ConfigMap's, and a ConfigMap can never satisfy it
...
| **mTLS** | Mutual TLS, where both sides of a connection present and verify a certificate |
...
For anything this guide doesn't answer, contact the Kubernetes hosted team: **#kubernetes-enterprise-operator** on Slack. If you hit something wrong or missing while using the guide, tell us there too. It only stays trustworthy if the field feeds it.
