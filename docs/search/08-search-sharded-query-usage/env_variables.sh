...
# is reusable across different sharded MongoDB Search deployments and all
# the necessary variables should be already defined there.
#
# Required env var contract:
#   MDB_ADMIN_CONNECTION_STRING — for import/sharding commands (admin privileges)
#   MDB_USER_CONNECTION_STRING  — for search queries (regular user privileges)
#   MDB_CONNECTION_STRING       — fallback if the above are not set (e.g., non-sharded usage)
#   K8S_CTX                    — Kubernetes context
#   MDB_NS                     — Kubernetes namespace
#   MDB_VERSION                — MongoDB version (e.g., 8.2.0-ent)
#   MDB_TLS_CA_CONFIGMAP       — CA certificate ConfigMap name for tools pod TLS mount
