...
echo "Waiting for search indexes to become READY (up to $((max_attempts * sleep_time))s)..."
...
for attempt in $(seq 1 ${max_attempts}); do
  ...
  echo "Attempt ${attempt}/${max_attempts}: movies='${movies_status}' embedded_movies='${vector_status}'"

  if [[ "${movies_status}" == "READY" && "${vector_status}" == "READY" ]]; then
...
echo "ERROR: Search indexes not ready after $((max_attempts * sleep_time)) seconds"
exit 1
