: "${IMAGE_PULL_SECRET_NAME:?not set -- source the env files first (see README, Environment section)}"
...
if [[ -z "${IMAGE_PULL_SECRET_NAME:-}" ]]; then
  ...
  for ctx in "${K8S_CLUSTER_0_CONTEXT_NAME}" "${K8S_CLUSTER_1_CONTEXT_NAME}"; do
    ...
    echo "[ok] Image pull secret '${IMAGE_PULL_SECRET_NAME}' created in cluster ${ctx}"
  done
fi
