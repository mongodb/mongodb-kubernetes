...
ca_data="$(kubectl --context "${K8S_CTX}" \
...
# Create ConfigMap for MongoDBCommunity TLS (keys: ca-pem, mms-ca.crt, ca.crt)
...
# Secret for external MongoDBSearch source TLS (expects ca.crt)
