...
metadata:
  name: ${MDB_RESOURCE_NAME_AUTO_EMBEDDING}
spec:
  version: ${MDB_VERSION}
  type: ReplicaSet
  members: ${MDB_MEMBERS}
  security:
    tls:
      ...
      certificateKeySecretRef:
        name: ${MDB_TLS_SERVER_CERT_SECRET_NAME_AUTO_EMBEDDING}
      caConfigMapRef:
        name: ${MDB_TLS_CA_CONFIGMAP_AUTO_EMBEDDING}
    authentication:
  ...
  users:
    ...
    - name: search-sync-source
      ...
      passwordSecretRef:
        name: ${MDB_RESOURCE_NAME_AUTO_EMBEDDING}-search-sync-source-password
      scramCredentialsSecretName: ${MDB_RESOURCE_NAME_AUTO_EMBEDDING}-search-sync-source
      roles:
