# Update MongoDB Sharded Cluster with search configuration
#
# This script updates the MongoDB cluster with search parameters pointing to
# the Envoy proxy endpoints. This must be run AFTER:
# 1. MongoDB cluster is running
# 2. Envoy proxy is deployed
# 3. MongoDBSearch resource is created (mongot services exist)
#
# Each shard's mongod needs to point to its corresponding Envoy proxy service.
# The mongos also needs search configuration to route search queries.

echo "Updating MongoDB cluster with search configuration..."
...
# Update the MongoDB Sharded Cluster with search configuration
...
echo "✓ MongoDB Sharded Cluster '${MDB_EXTERNAL_CLUSTER_NAME}' updated with search configuration"
