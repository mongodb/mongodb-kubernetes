...
# The operator derives it via LoadBalancerServiceName
# (api/v1/search/mongodbsearch_types.go) using the hardcoded pattern:
#   {search-resource}-search-lb-svc
# Changing this var won't change the real Service — it will just break the scripts.
export MDB_PROXY_HOST="${MDB_RESOURCE_NAME}-search-lb-svc.${MDB_NS}.svc.cluster.local:${ENVOY_PROXY_PORT}"
