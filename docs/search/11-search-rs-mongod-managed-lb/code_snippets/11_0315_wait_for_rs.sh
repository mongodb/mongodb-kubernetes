#!/usr/bin/env bash
# Wait for the operator-managed MongoDB replica set to be ready
...
if [[ ${elapsed} -ge ${timeout} ]]; then
  echo "ERROR: Timeout waiting for MongoDB replica set to be ready"
...
kubectl get pods -n "${MDB_NS}" --context "${K8S_CTX}" -l "app=${MDB_RESOURCE_NAME}-svc" --no-headers

echo ""
echo "✓ Operator-managed MongoDB replica set is ready"
