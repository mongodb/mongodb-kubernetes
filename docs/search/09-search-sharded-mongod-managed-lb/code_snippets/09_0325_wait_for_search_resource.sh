...
while [[ ${elapsed} -lt ${timeout} ]]; do
  phase=$(kubectl get mongodbsearch "${MDB_RESOURCE_NAME}" \
    -n "${MDB_NS}" \
    --context "${K8S_CTX}" \
...
if [[ ${elapsed} -ge ${timeout} ]]; then
  ...
  kubectl describe mongodbsearch "${MDB_RESOURCE_NAME}" -n "${MDB_NS}" --context "${K8S_CTX}"
  echo ""
  echo "Search pods:"
...
echo ""
echo "✓ MongoDBSearch '${MDB_RESOURCE_NAME}' is ready"
