...
graph TD
    subgraph k8s["Kubernetes Cluster"]
        subgraph mdb["Operator-Managed MongoDB Sharded Cluster"]
            mongos(["mongos\nquery router"])
            s0(["shard-0 mongod"])
            s1(["shard-1 mongod"])
            cfg(["config servers"])
    ...
    style cfg fill:#00684A,stroke:#fff,color:#fff
...
|-------------|---------------------|---------|
| Server cert | `{prefix}-{name}-search-lb-cert` | Presented to mongod during TLS handshake (wildcard SAN covers all shards) |
