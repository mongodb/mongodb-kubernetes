#!/usr/bin/env bash
# Wait for the simulated external MongoDB replica set to be ready
...
echo ""
echo "Cluster pods:"
kubectl get pods -n "${MDB_NS}" --context "${K8S_CTX}" -l "app=${MDB_EXTERNAL_CLUSTER_NAME}-svc" --no-headers

echo ""
echo "✓ Simulated external MongoDB replica set is ready"
