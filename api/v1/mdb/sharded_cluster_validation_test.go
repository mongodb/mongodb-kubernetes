...
func TestNonEmptyShardClusterSpecItemRemoval(t *testing.T) {
	scOld := NewDefaultMultiShardedClusterBuilder().Build()

	scNew := NewDefaultMultiShardedClusterBuilder().Build()
	// Removing `test-cluster-0` with 2 members from the ClusterSpecList
	scNew.Spec.ShardSpec.ClusterSpecList = ClusterSpecList{
		{
	...
	_, err := scNew.ValidateUpdate(scOld)

	require.Error(t, err)
	assert.Equal(t, "Cannot remove shard cluster test-cluster-0 with non-zero members count. Please scale down members to zero first", err.Error())
}

func TestNonEmptyConfigSrcClusterSpecItemRemoval(t *testing.T) {
	scOld := NewDefaultMultiShardedClusterBuilder().Build()

	scNew := NewDefaultMultiShardedClusterBuilder().Build()
	// Removing `test-cluster-0` with 2 members from the ClusterSpecList
	scNew.Spec.ConfigSrvSpec.ClusterSpecList = ClusterSpecList{
		{
			ClusterName: "test-cluster-1",
			Members:     3,
		},
	...
	require.Error(t, err)
	assert.Equal(t, "Cannot remove configSrv cluster test-cluster-0 with non-zero members count. Please scale down members to zero first", err.Error())
}

func TestNonEmptyMongosClusterSpecItemRemoval(t *testing.T) {
	scOld := NewDefaultMultiShardedClusterBuilder().Build()

	scNew := NewDefaultMultiShardedClusterBuilder().Build()
	// Removing `test-cluster-0` with 2 members from the ClusterSpecList
	scNew.Spec.MongosSpec.ClusterSpecList = ClusterSpecList{
		{
			ClusterName: "test-cluster-1",
			Members:     3,
		},
	...
	require.Error(t, err)
	assert.Equal(t, "Cannot remove mongos cluster test-cluster-0 with non-zero members count. Please scale down members to zero first", err.Error())
}
