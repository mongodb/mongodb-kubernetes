...
func TestPublishImages(t *testing.T) {
	...
	results, err := PublishImages(images, "abc1234", "latest", "", false, false, true, nil, insecureConnect)
	if err != nil {
...
func TestPublishImagesRegistryOverride(t *testing.T) {
	...
	results, err := PublishImages(images, "abc1234", "latest", override, false, false, true, nil, insecureConnect)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(results) != 1 {
...
func TestPublishImagesHardFailsOnMissingPromotedTag(t *testing.T) {
	...
	_, err := PublishImages(images, "abc1234", "latest", "", false, false, true, nil, insecureConnect)
...
func TestPublishImagesCommitRequired(t *testing.T) {
	...
	_, err := PublishImages(images, "", "", "", false, false, true, nil, insecureConnect)
...
func TestPublishImagesRefusesAllOnAnyConflict(t *testing.T) {
	...
	_, err := PublishImages(images, "abc1234", "latest", "", false, false, true, []string{"operator", "readiness-probe"}, insecureConnect)
	if err == nil || !strings.Contains(err.Error(), "readiness-probe") {
	...
	results, err := PublishImages(images, "abc1234", "latest", "", true, false, true, nil, insecureConnect)
	if err != nil {
